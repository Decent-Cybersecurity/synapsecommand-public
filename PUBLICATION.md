...
| `7031ab1f` | 2026-09-07 12:05:26 | `a0cc8967` | **did not exist** — deployed by this round from the `v2.0.0` tree, on ledger entry 18 |
...
`18:22:37Z`, before that
`2fc4ab7b` for twenty-six hours and twelve
minutes until the 2.0.0 witness round superseded it at `12:05:26Z`, before that `6ec27291` for eleven
hours and twenty-six
...
### 18. `synapse-cdm` 2.0.0 is on the index — CLOSED, and it is the first release this package has made that a 1.x reader is meant to refuse

**Published 2026-09-07** by
[run 34117343838](https://github.com/Decent-Cybersecurity/synapsecommand-public/actions/runs/34117343838),
triggered by the `v2.0.0` tag at `a0cc896` — tag object `a0ce1933`, annotated, tagger Matej Michalko,
created **11:32:45Z**. The run was created at 11:34:50Z by the push; the build job ran
11:34:53–11:38:00Z, three minutes and seven seconds; the `pypi` environment then held the upload for
**seventeen minutes and forty-five seconds** until it was approved at **11:55:47Z**, and the publish
job ran 11:55:49–11:56:06Z, seventeen seconds. Deployment `6307853388` carries the sequence:
`waiting` at 11:38:02Z, `queued` at 11:55:47Z, `in_progress` at 11:55:49Z, `success` at 11:56:07Z.
**The second act, tag push to `success`, took twenty-one minutes and seventeen seconds**, of which
the hold was seventeen minutes and forty-five.

**THE WIRE CONTRACT MOVED, AND THIS ENTRY'S SUBJECT IS WHAT A VERSION NUMBER CAN AND CANNOT SAY.**
Eight package releases shipped without `SCHEMA_VERSION` moving; this one moves it, and the two
numbers land on `2.0.0` together. `Event.oes` and `Entity.ontology_types` are both OPTIONAL and both
default to nothing, so legacy data is structurally representable without change — **and that is not
compatibility.** The canonical objects are `extra="forbid"` and the published schemas are
`additionalProperties: false`, so a 1.x strict reader meeting either key **refuses the object rather
than ignoring it**; `version.compatible("2.0.0", "1.0.0")` is `False`, and that refusal is the
message the major exists to carry. Nothing was removed, renamed or narrowed, and no migration tooling
was written: `MIGRATIONS.md`'s 2.0.0 section carries the migration statements instead. **A package at
2.0.0 does not mean SC-OES 2.0** — `SC_OES_VERSION` is `0.1.0` and it is a Draft.

**THE NUMBER IS A PERSON'S OVER THE GATE'S FLOOR, AND THE GATE SAYS SO ITSELF.** Read on this tree,
`gates/bump_derivation.py` reports `declared 2.0.0`, `declared_kind MAJOR`, `derived_kind MINOR`,
with the version rule `MAJOR` ruled for `1.8.0 → 2.0.0`. It derives its floor from **697 signals
across 558 distribution files** — and 558 is also the number of files `git diff --name-only
v1.8.0..HEAD -- packages/cdm` reports, so **every file the arc moved carries at least one signal**.
Every signal it can prove is an addition: an optional field on two models, a widened `__all__`, three
shipped registries, an adapter emitting a key it did not emit before. No importable name is removed
and no signature moves, so **no MAJOR row is reached by the diff**. What decides the number is what
the diff cannot reach — a third party's consumer written against 1.8.0 does not work against this
distribution, and no file in this distribution records a third party's code. `docs/adr/0005-cdm-schema-version-impact.md`
argues it and `MIGRATIONS.md`'s 2.0.0 section carries the ruling. **Ten units in the arc carry a
person's ruling** — seven MINOR and three PATCH — and `pending.unruled` is the empty list, which is
the pre-step the procedure requires before a number is typed. The gate calls its own answer a FLOOR
and accepts a declared version above it **only** where the arc's section carries the Version ruling;
that is the mechanism, not a bypass, and this is the first release to exercise it.

**NO TAG MOVED ON ORIGIN, AND ONE WAS REMADE LOCALLY BEFORE ANYTHING WAS PUSHED.**
`git ls-remote --tags origin v2.0.0` answers `a0ce1933`, which is the object this tree holds;
`gh run list --branch v2.0.0` lists **one** run; the `pypi` environment holds **one** deployment for
the tag, `6307853388`, at `a0cc896`. The remade tag is entries 15, 16 and 17's shape and it was
remade for the same reason: `tests/test_cdm_bump_derivation.py` reads the arc `v1.8.0 → v2.0.0` the
moment the tag exists, so the `UNRULED_HISTORICAL_ARCS` row naming that arc's ten units cannot be
written before the tag and cannot be missing after it. The release commit is `fcde019` and moves
**one** path — `packages/cdm/synapse_cdm/MIGRATIONS.md`, `93 insertions(+), 8 deletions(-)`;
`a0cc896` adds forty-one lines to `tests/test_cdm_bump_derivation.py` and moves nothing else, and
`git diff fcde019 a0cc896 -- packages/cdm` is empty, so **the tagged commit's distribution is the
release commit's distribution**. `v2.0.0` is the twelfth tag.

**WHAT APPROVED IT WAS A VERDICT, FOR THE THIRD TIME, AND THE MECHANISM IS ENTRY 16'S.** The release
round ran to the publish hold and stopped there; a second session re-derived every figure its report
stated against the tree at `a0cc896` and returned `GO`; and only then did the runner `POST` to the
run's `pending_deployments` — issued at **11:55:46Z** by this machine's clock, one second before the
deployment's `queued` stamp, with the comment *"2.0.0, approved by the runner after round-reviewer GO
(rounds/reports/RL.review.md)"* naming the verdict file. The `pypi` environment still has exactly one
required reviewer, `decentcybersecurity`, and this entry makes no claim that anything about the
*protection* is different. The hold was **seventeen and three-quarter minutes** against 1.8.0's
thirteen and 1.7.0's twenty-one and a half; three holds on a verdict have now taken between thirteen
and twenty-two minutes, which is a range and not a figure.

**The digests, in six readings, and all six were reachable.** What the gate hashed after building and
gating, what the publish job hashed immediately before uploading, what the in-toto attestation names
as its subject, what the simple index declares, what the legacy JSON metadata states, and what a
recomputation over the downloaded bytes yields:
...
**All six agree, and the sizes agree with them** — 6 880 796 bytes for the wheel and 4 905 158 for the
sdist, identical in the legacy JSON, in the simple index's JSON form, and in the files downloaded from
the index on **2026-09-07 at 11:59:37.335Z and 11:59:38.428Z**. The gate hashed them at
**11:36:49.685Z** in the Condition 2 step, in the same output that reports `harness PASS 14 adapters x
2 schema modes, 1076 fixture verdicts, 0 failed` and `13 checks, 0 failed`, and the publish job hashed
the same two values at **11:55:51.800Z** and **11:55:51.804Z**, across the hold. The in-toto statements
were read twice: in the publish job's own log as the DSSE payloads it signed at 11:55:59.329Z (sdist)
and 11:55:59.972Z (wheel), and from the bundles the simple index links, fetched at **11:59:33.905Z** and
**11:59:34.098Z**. **A seventh reading is the Condition 4 echo** — the two digests appear again at
**11:36:50.180Z** and **11:36:50.182Z** under `### Artefacts, with digests`, and agree — and **an eighth
is the publish action's own `print-hash`**, at **11:56:00.601Z** and **11:56:00.621Z**, after the
attestations were generated and before the upload; it is named here because a reader counting `SHA256:`
lines in that step's log would find four and not two. **The comparison basis is the WORKFLOW's build and
never a local rebuild**, on entry 11's demonstration, inherited rather than re-argued.

**FOUR DIGESTS IN THIS RUN ARE NOT READINGS OF EITHER FILE, and they are named so a reader does not
count them among the six.** `sha256:6dbcfb49b54c9c38dd8a27b5a2a0c8b14dc3cdee9a5a1e52ff18db32ab724cdc`
is the digest of **GitHub's artifact bundle** — 9 475 862 bytes, artifact `10016878647`, declared as
the expected digest by the download step at 11:55:51.314Z and confirmed over the received bytes at
11:55:51.754Z; it is a property of a container and appears in no PyPI metadata.
`e8b6847955c9a6af40fc21e3831eb3daee8cb70087048af3fb60b569d13fce1c` and
`e2beb3125438af6a1cdee759a61f8bc314cf0757897da5e2b834e3c880ad7bd0` are the two `.publish.attestation`
files uploaded alongside the distributions — sdist's and wheel's, in that order. And
`a68d0551…c4e3ab2d5` is the digest of the **container image** the publish action ran,
`ghcr.io/pypa/gh-action-pypi-publish` at `dc37677b2e1c63e2034f94d8a5b11f265b73ba33` — the same image,
to the digest, that entries 14, 15, 16 and 17 pulled.

**The probe protocol was applied rather than assumed.** Every request carried a declared
`User-Agent`, non-200 raised before any body was touched, and each downloaded file was checked for its
archive magic — `PK\x03\x04` for the wheel, `\x1f\x8b\x08\x08` for the sdist — **before** it was
hashed. Both checks passed. No non-200 was met at all, including at the changelog URL, which was
requested with its trailing slash throughout. PyPI's receipt stamps are **11:56:01.834740Z** for the
wheel and **11:56:03.752994Z** for the sdist; the download that recomputed the digests ran three
minutes and thirty-four seconds after the second of them, and as in entries 16 and 17 **no latency
figure is offered beside them** — this round fetched the provenance bundles first.

**THE STALE 200 DID NOT REPRODUCE, WHICH IS THE SECOND TIME IT HAS NOT.** `GET /simple/synapse-cdm/`
at **11:59:15.289Z**, three minutes and eleven and a half seconds after the sdist's receipt stamp,
returned **twenty-four** anchors whose last two are **2.0.0's**, each with a `data-provenance` URL and
the two digests above, and a trailing serial of **40818482**; the JSON form read 198 milliseconds
later agreed and listed **twelve** versions ending `2.0.0`. So the lag entries 14, 16 and 17 caught —
bounded by them between two and three-quarters and twelve minutes — is **absent at three and a
quarter**, as it was absent for entry 15 at eight. **Four releases have now bounded this endpoint's
lag and two have failed to find it at all, which is not a measurement and is recorded as one more
point rather than as a conclusion.**

**A DIFFERENT PyPI ENDPOINT DID LAG, AND THIS IS THE FIRST TIME THIS FILE RECORDS IT — AS A BOUNDED
OBSERVATION AND NOT A MEASUREMENT.** `GET /pypi/synapse-cdm/json`, the legacy API's *latest-version*
form, answered `info.version` **1.8.0** in a read this round can only bound to between **11:56:20Z and
11:57:19Z** — after both receipt stamps — and **2.0.0** at **11:59:25.196Z**. **That first read was
taken with `curl` outside the probe protocol and was not dated to the second, and it is written down
with those limits rather than tidied into a measurement**: what it establishes is that the
per-version form (`/pypi/synapse-cdm/2.0.0/json`, `200` with two files, read 11:59:25.117Z) and the
latest-version form of the same API can disagree for some interval after an upload. No length is
claimed for that interval and no other release has been read this way.

**THE PyPI PROVENANCE TRAP, NINTH READING, AND IT READS AS THE FIFTH THROUGH EIGHTH DID.** Read
**2026-09-07T11:59:25.117Z**, the two per-file objects for 2.0.0 in `GET /pypi/synapse-cdm/2.0.0/json`
carry **no `provenance` key at all** — sixteen keys each, and `provenance` not among them. Only 2.0.0
was read this time; entry 14's sweep of every release and a control package is the reading that makes
this a property of the endpoint. The attestations are where they have been for nine releases: the
simple index's per-file `provenance` URL, and the bundles fetched from them at 11:59:33.905Z and
11:59:34.098Z state the four trusted-publisher values — `kind` **GitHub**, `repository`
**Decent-Cybersecurity/synapsecommand-public**, `workflow` **publish.yml**, `environment` **pypi** —
with in-toto subjects carrying the two digests above and predicate type
`https://docs.pypi.org/attestations/publish/v1`. Look for the key, not for its value.

**THREE SUITE TOTALS THAT DIFFER, AND ALL THREE TOTAL 4507.** Reconciled 2026-09-07:

| reading | passed | skipped | failed | total |
| --- | --- | --- | --- | --- |
| CI, at the tag, 11:35:06–11:36:13Z | 4429 | 78 | 0 | 4507 |
| fresh clone at `v2.0.0`, this machine, 12:00:17–12:01:48Z | 4430 | 77 | 0 | 4507 |
| maintainer's tree at `a0cc896`, 12:12:14–12:13:43Z | 4496 | 8 | **3** | 4507 |

**Clone versus maintainer is seventy-one tests, and the passed counts differ by sixty-six.** **Seventy**
tests skip in the clone and run here — **42** in `test_cdm_pins.py`, 10 in `test_cdm_format_coverage.py`,
7 in `test_cdm_pin_paths.py`, 6 in `test_cdm_stanag4609_adapter.py`, 2 in `test_cdm_pdf_text.py`
(`:211`, the pinned ST 1303.1, and `:353`, the pinned RFC 2781 the arc since 1.8.0 added as a `.txt`),
and one each in `test_cdm_parks_table.py`, `test_cdm_stanag4586_adapter.py` and
`test_cdm_version_floor.py` — while **one** test skips here and runs in the clone,
`test_cdm_pdf_text.py:221`, *"no PDF reader in this environment"*. Sixty-nine of the seventy are the
gitignored bytes — the pinned specification documents and the KLV streams — and the seventieth is
`test_cdm_version_floor.py:368`, which skips because there is no virtualenv **inside** the clone.
**Seven** skips are common to both readings: `test_cdm_ordinals.py:495`, `test_cdm_pins.py:756`, and the
**five** `test_cdm_release.py` gates that skip when the package tree is identical to its tag — the same
seven, at the same lines, entries 15, 16 and 17 found. The arithmetic closes: 4430 + 70 − 1 − 3 = 4496.

**THE FRESH-CLONE FIGURE THIS ROUND TOOK IS ONE TEST AWAY FROM THE ONE ITS OWN REVIEW TOOK, AND THE
DIFFERENCE IS WHERE THE ENVIRONMENT SITS RATHER THAN WHAT THE TREE HOLDS.** The release half's review
read **4431 passed, 76 skipped** in a fresh clone of this same commit; this round read **4430 passed,
77 skipped**. Both total 4507 and the offender is
`test_cdm_version_floor.py::test_the_environment_exclusion_is_not_vacuous_in_this_tree`, which walks
the clone for a `pyvenv.cfg` and **skips rather than fails** where it finds none, because "an outsider
running the suite against a system interpreter has no environment inside the clone and that is a legal
way to run it". This round put the virtualenv beside the clone; the review put one inside it. **Proved
rather than inferred**: with a throwaway `.venv` created inside the same clone the test reads **1
passed** (12:03:20Z), and with it removed it skips. Two legal ways to site an environment give two
legal readings of one collection, and a table of three totals cannot be reconciled without saying
which was used.

**The maintainer's reading was taken over this round's own edits, deliberately** — 12:12:14–12:13:43Z,
with entry 18 written and the test module's two count sites moved — because a round report and a
release record are both inside the sweeps' scope. **It was taken twice and reads identically** —
again at 12:16:56–12:18:30Z, over the entry with its own suite table filled in, `3 failed, 4496
passed, 8 skipped` both times and the same three names. **What proves this round added no offender to
the failures below is the offender lists themselves**: every site in all three is an untracked file under
`rounds/`, and the two files this round writes are tracked. `git ls-files rounds/` returns **0**.

**The three failures are not in the distribution, are not in the clone, and are not in the commit.**
`tests/test_cdm_changelog_claim.py:136` names **59 unique offender sites across 40 files** (60
occurrences — one file carries the same line twice); `tests/test_cdm_consumer_path.py:136` names **16**,
every one a `rounds/reports/*.md`; and `tests/test_cdm_deploy_workflow.py:233` reports `only in the
list: []` and `only in the tree` **two** files, `rounds/briefs/RL-release-2.0.0.md` and
`rounds/reports/I2.review.md`. All three walk the **filesystem** rather than the git index, and every
site they name is inside the untracked directory the two sessions of this round use to pass work
between them: none is tracked, none is in the tarball or the wheel, and the same three tests at the
same commit **pass** in the fresh clone above — `3 passed` where the tree reads `3 failed` — which is
...
found** and `GET /releases/tags/v2.0.0` answered **404** at **12:00:17Z**, with `releases/latest` still
naming v1.8.0. Six of the last seven releases have needed one made by hand. Created at
**2026-09-07T12:03:26Z** (`published_at`) by `decentcybersecurity`, id `384058769`, not a draft, not a
prerelease, now `Latest`, target `main`, zero assets, with a body derived from `RELEASE_NOTES.md` and
the run's own derivations — fourteen adapters, 538 fixture verdicts, six schemas, thirty pins, 4429
passed and 78 skipped at the tag, 697 signals across 558 files, ten ruled units, and the two digests.
**Its `created_at` field reads 11:32:45Z and that is not when it was made** — it is `a0ce1933`'s tagger
time to the second, on the mechanism entry 13 records, so `published_at` **12:03:26Z** is the instant
that matters. Sixth reproduction. The body as published is **byte-identical to the intended one**,
6 578 bytes, checked by comparing the API's `body` against the file it was created from rather than by
reading it; no edit was needed and none was made, so the amendment rule entry 17 exercised does not
arise here.

**THE DOCUMENTATION SITE WAS ONE RELEASE BEHIND AND THE WITNESS READ IT, WHICH IS NOW THE ORDINARY
COURSE.** Measured **2026-09-07T11:59:39.219Z**: <https://docs.synapsecommand.com/changelog/> returned
HTTP 200, 58 817 bytes, digest `bef6e2c3…64b5` — the bytes the 1.8.0 witness round deployed — while
the index served 2.0.0. `gates/deploy_record.py`, run 11:57:19–11:57:25Z, reconciled clean —
twenty-two deployments, 0 unaccounted for, alias on `2fc4ab7b` — **and its served-version witness read
`states 1.8.0 and version.py declares 2.0.0 — DISAGREE`**, its fourth DISAGREE and its fourth release.
It cannot fail the gate — `2 checks, 0 failed; 1 witness, which cannot fail` — and did not.

**Built from the clean tree at `a0cc896` before any record was written, so the stamp is the tag's
commit.** `docs/build` and `docs/.docusaurus` were removed first, and the block in
[`docs/README.md`](docs/README.md) was run as written, **12:04:25Z–12:04:27.997Z**:
`check-schema-docs` reported *CURRENT — 9 generated files match the schemas in schemas*, the generator
*0 written, 9 already current*, and `check:admonitions` *OK — 17 directives in the sources, 17
admonitions rendered, 0 literal ':::' in 19 built pages*. The tree was still clean afterwards —
`git status --porcelain` empty — and `docs/package-lock.json` was not rewritten. **The build was run a
second time, 12:05:02.957–12:05:04.882Z, from the same clean tree**, and it produced the
**byte-identical** page — 61 070 bytes, `f958bae1…b5f00`, 52 files including `.nojekyll` — so what is
quoted above is reproduced and not transcribed once. **The start of the first window is dated to the
second and not to the millisecond, and it is said to be**: BSD `date` on this host has no `%3N`, so the
first stamp was taken at second resolution and the window's end is the log file's own mtime. The
second window is millisecond-dated at both ends. **The built page and the page it replaced differ in
size as well as in bytes** — 61 070 against 58 817, `f958bae1…b5f00` against `bef6e2c3…64b5` — which
is the first release in four where the sizes did not coincide; every comparison here is by digest
regardless, for the reason entries 15, 16 and 17 give.

**A DATED SENTENCE ON THE PAGE THIS ROUND DEPLOYED STOPPED BEING TRUE FORTY MINUTES BEFORE THE DEPLOY,
AND THIS ROUND IS NOT AUTHORISED TO CORRECT IT.** Inside the changelog's admonition, in a paragraph
dated **2026-09-07**, `docs/docs/changelog.mdx` states *"**Still no release carries either change**:
the index serves 1.8.0, and `pip install synapse-cdm` resolves it"* — written this morning on the
working branch, true when written, and **false from 11:56:03.752994Z**, the sdist's receipt stamp. The
sentence is in the built page once and is therefore now served. The **served-version witness is
unaffected** and reads `2.0.0`, because what it parses is the sentence two lines above it (*"the
package is at `2.0.0`"*), which the tree made true and the deploy published. Correcting the false
sentence is an edit to a tracked file no ruling of this round names, so it is **recorded here and
left**, under the standing rule that a dated statement which has become false gets a dated correction
beside it and never a silent edit. It is entry 18's second finding.

**Deployment `7031ab1f`, source `a0cc896`, uploaded 12:05:11.073–12:05:26.582Z** — 36 files of 52 new,
16 already held, wrangler 4.129.0 — preview <https://7031ab1f.synapsecommand-docs.pages.dev>. **The two
instants are client-side and are said to be**: Cloudflare's list reports ages rather than timestamps
through the CLI, so what is written here is when this machine started and finished the upload. The
deployment's own source commit is not client-side and is not taken on trust — `gates/deploy_record.py`
read it back off Cloudflare as `a0cc896` at 12:05:45–12:05:48Z, in the act of refusing the deployment
this file could not yet name, *"24 seconds ago"*.

**The alias served the new page eleven seconds after the upload finished.** At **12:05:37.894Z**
<https://docs.synapsecommand.com/changelog/> was **byte-identical** to the local build — same 61 070
bytes, same `f958bae1…b5f00` — and the deployment's own `pages.dev` URL at 12:05:38.117Z read the same.
Entry 14 caught the alias still answering from cache eight seconds after its upload; entry 15's first
reading was twenty seconds after and current, entry 16's three, entry 17's five, and this one eleven.
**Five readings, one of them stale, and none of them taken twice** — still not a distribution, and
recorded for entry 16's reason: a first reading is the one a round could mistake for a failed deploy.

**The gate refused once, and the refusal is the record working.** At **12:05:45–12:05:48Z**, with no
edit made, it named the deployment this file could not name. After the row and the count moved and the
alias paragraph was rewritten it read, at **12:11:09–12:11:15Z**, **twenty-three deployments, 0
unaccounted for, alias `7031ab1f`, source `a0cc896`, and the served-version witness `states 2.0.0 and
version.py declares 2.0.0 — AGREE`** — the same witness that read DISAGREE at 11:57:25Z, eight minutes
earlier, on the same tree. This is the **eighth** time the gate has caught its own round's upload, and
the paragraph and the row moved in one edit, so entry 15's second refusal did not arise.

**FIVE ROWS OF THE SWEEP TABLE BELOW HAD NOT MOVED SINCE 1.6.0, AND THIS ROUND STRIKES THEM RATHER
THAN ADDING A SIXTH SILENT READING. It is entry 18's first finding.** The alias row read
`5a908754`, superseded three times since — by `6ec27291` at 2026-09-05 22:26:51Z, `2fc4ab7b` at
2026-09-06 09:53:41Z, and `7031ab1f` today — and the version row read *the distribution on the index
is 1.6.0*, superseded by 1.7.0, 1.8.0 and now 2.0.0. The provenance row stopped at its **sixth**
reading and the OIDC row at its **eighth** witness, while entries 16 and 17 recorded a seventh and an
eighth reading and a ninth and a tenth witness in their own prose; the served-version row stopped at
entry 15's three readings while entry 17 recorded two more. **Every one of those readings was taken —
none of them reached this table**, which is the habit the 1.4.0 version row already names one column
over: *"two releases shipped without this table moving"*. It has now happened twice, and the second
time it was two releases again. The rows are struck and dated here, the supersessions are attributed
to the entries that recorded them rather than re-measured, and **the only new readings in those five
cells are this round's own**. What would close it is a gate, and there is none: nothing derives this
table's currency from the entries above it. That is recorded as the finding rather than repaired,
because writing one is outside this round.

**The five tree untouchables hold, each by its own command, and none of them moved.** The pinned phrase
derives to **35** over the git index, in 26 files; `scripted_edit`'s contract is green at **9**;
`git ls-files` matches **no** PDF and **no** zip, and `fixtures/*/spec/` tracks only `.json` and `.py`,
nine of each; and `klv_pin.json`'s delegation tally still reads fourteen, in exactly one file. **The
sixth untouchable is non-empty at this release by design and is recorded rather than waived**:
`git diff v1.8.0..HEAD -- schemas/` is **6 files, 884 insertions(+), 20 deletions(-)**, because the
schema is this arc's subject — `python -m synapse_cdm.schemas --check --out schemas` reads
`CURRENT: schemas vs models at 2.0.0` and the workflow's gate regenerated all six from outside the
repository byte-identical, which is what makes it a publication of the models rather than drift.
`gates/parks_table.py` reads **13 rows — 0 open [], 13 closed [1…13]**, `0 set-claims, 0 failed`, the
second release running with an empty table, and `gates/pin_paths.py` reads `30 present, 30 matched, 30
copies, 0 failed` — **the set 1.8.0 shipped, unmoved**: `git diff --name-only v1.8.0..v2.0.0 --
'*_pin.json'` is empty, which is a derivation and not the comparative entry 17 had to correct.
**This round writes two files** — this one and the test module that states the ledger count, whose
docstring and whose number vocabulary both had to move, the vocabulary because it ended at seventeen
and the entry this round adds is the eighteenth. Nothing under `packages/cdm/` moved at all:
`version.py`, `RELEASE_NOTES.md`, `klv_pin.json` and `FORMAT_COVERAGE.md` are byte-identical to
`a0cc896` at the close, verified by `git hash-object` rather than by intent.

**What this entry does not claim.** That the approval says anything about token state: the row reading
**UNDATABLE from held evidence** stands untouched, and this upload is the eleventh dated witness that
the tokenless mechanism works and the eleventh that is silent about what else would be accepted. It
does not claim the simple index's lag has a length: four releases have bounded it and two have not
found it. It does not claim any length for the legacy API's latest-version lag either — one bounded
observation is not an interval. It does not claim PyPI's `provenance` key is absent on any release but
2.0.0 today. It does not claim the docs site is gated: the served-version witness reports and cannot
fail. It does not claim that a reviewing session is a person. It does not claim SC-OES is a standard,
adopted, or certified — the specification is v0.1.0 **Draft**, six of its seven profiles have no
executable rule, and this work creates no certification programme. And **it does not claim the two
`2.0.0`s mean one thing**: the package's number and the wire contract's number were derived
separately, by different rules, in different documents, and their landing on one value is a
coincidence this release is at pains to say is not a derivation.

### 19. `synapse-cdm` 2.1.2 is on the index — CLOSED, and it is the first release this package has made after two of its own tags were refused by its own gates

**Published 2026-09-12** by
[run 34687815710](https://github.com/Decent-Cybersecurity/synapsecommand-public/actions/runs/34687815710),
triggered by the `v2.1.2` tag at `5ab80f5f` — tag object `e8fed432`, annotated, tagger Matej Michalko,
created **09:57:57Z**. The run was created at 10:12:05Z by the push, fourteen minutes and eight seconds
after the tag object was written, and that gap is the release procedure rather than a delay: the tag is
made locally, the branch is pushed alone, the commit's code-scanning analyses are waited for and read,
the ref rehearsal is run against them, and only then are `main` and the tag pushed together. The gate
job ran 10:12:08–10:19:11Z, the build job 10:19:14–10:27:35Z and the attestation job
10:27:38–10:28:07Z; the `pypi` environment then held the upload for **twenty minutes and forty-three
seconds** until it was approved at **10:48:52Z**, and the publish job ran 10:48:54–10:49:14Z, twenty
seconds. Deployment `6408618929` carries the sequence: `waiting` at 10:28:09Z, `queued` at 10:48:52Z,
`in_progress` at 10:48:54Z, `success` at 10:49:15Z. **The second act, tag push to `success`, took
thirty-seven minutes and ten seconds**, of which the hold was twenty minutes and forty-three.

**TWO EARLIER TAGS NAME COMMITS THE INDEX HAS NEVER SEEN, AND THIS ENTRY'S FIRST SUBJECT IS WHY THEY
STAY WHERE THEY ARE.** `v2.1.0` is tag object `e2c05107`, written 2026-09-09T08:30:35Z over commit
`b69a267e`; its run
[34332384035](https://github.com/Decent-Cybersecurity/synapsecommand-public/actions/runs/34332384035)
was created 09:01:45Z and concluded **failure** in the gate job, at step **15, `Security gate —
pip-audit, strict`**, with every job after it `skipped`. `v2.1.1` is tag object `68d3a3c4`, written
2026-09-10T07:41:56Z over commit `4409115b`; its run
[34452755466](https://github.com/Decent-Cybersecurity/synapsecommand-public/actions/runs/34452755466)
was created 07:59:26Z and concluded **failure** in the same job, at step **16, `Security gate —
CodeQL, over the analysis this commit already has`**, again with everything after it `skipped`.
**Neither ever reached PyPI**, and that is read rather than inferred: `GET
/pypi/synapse-cdm/2.1.0/json` and `GET /pypi/synapse-cdm/2.1.1/json` both answer **404**, and the
legacy API's own release list runs `1.0.0, 1.1.0, 1.2.0, 1.2.1, 1.3.0, 1.4.0, 1.4.1, 1.5.0, 1.6.0,
1.7.0, 1.8.0, 2.0.0, 2.1.2` with neither number in it. **Both tags remain permanently where they
are.** Nothing was moved, deleted or re-pointed to make this release; `git rev-parse` reads
`v2.1.0` at `b69a267e` and `v2.1.1` at `4409115b` on the day 2.1.2 was witnessed, and a reader who
finds either tag finds a commit that was built, gated and refused. **2.1.2 is the corrective, and it
is the first SOIF Part 1 release this package has successfully published.**

**Both refusals were a gate refusing its own release candidate, and they are not the same defect.**
The 2.1.0 refusal is a resolution failure: `pip-audit --strict` resolves every installed distribution
against the index, and the candidate is by definition not on the index yet, so the step asked for a
publication it was gating. The 2.1.1 refusal is a ref failure: the step asked the code-scanning API
for the analyses of `GITHUB_REF`, and no workflow in this repository scans a tag ref, so on a tag push
the query can only come back empty. **A branch dispatch passes that step both before and after the
repair**, which is why the second tag was spent finding out what the first one had not.

**The digests, and all six were reachable from the Release alone.** `SHA256SUMS` names
`56eddc85ce26cc484636f9352042c391d78b71d4ccb22aeaec048f245c645e51` for the wheel,
`ea52f49146611dccf0d102cefe77bad3132be54249e945edbb6eb3e7333d5a37` for the sdist,
`b258a36f698afefd1ff49dfad80064b134fd8c41630534a86915e18f96406883` and
`243ef7a51c1adfa1d29efca2835abd3b2e7cdf21bdc20c181d1cb56230536dd7` for the CycloneDX and SPDX
SBOMs, `d3f2d75e8e6cd6f8a62296c14ad19408de3f85638ddb328e0216abe849a27626` for the conformance sweep
and `6ff16b5eeeda78e32a41f70807a6141cfa15822d72d881302243d621ef73614a` for the evidence bundle.
**The index agrees about the two that reach it**: `GET /pypi/synapse-cdm/2.1.2/json` returns the same
wheel and sdist digests, with sizes 7 166 639 and 5 121 597 bytes and upload times
**10:49:09.492298Z** and **10:49:11.553329Z** — one hundred and ninety-eight milliseconds apart, and
the second of them is the instant this release became a thing a stranger could install. The Release
body is byte-identical to the `release-notes-2.1.2.md` asset,
`f00224687443aea426eae274a31f10a7078496d4354dbb69630a8e4202d42469` over 39 178 bytes on both.

**THE PyPI PROVENANCE TRAP, TENTH READING, AND IT READS AS THE FIFTH THROUGH NINTH DID.** Read
2026-09-12T18:24Z, the two per-file objects for 2.1.2 in `GET /pypi/synapse-cdm/2.1.2/json` carry no
`provenance` key at all — probed for the KEY and not for a value, because the endpoint stopped
carrying it rather than started answering `null`. **The attestations are on the simple index, and
they witness the publisher from PyPI's own side.** `GET /simple/synapse-cdm/` with
`Accept: application/vnd.pypi.simple.v1+json` gives each 2.1.2 file a `provenance` URL; the bundle at
the wheel's names `kind` **GitHub**, `repository` **Decent-Cybersecurity/synapsecommand-public**,
`workflow` **publish.yml**, `environment` **pypi**, and its in-toto statement's
`subject[0].digest.sha256` is `56eddc85…c645e51`, the wheel's own digest. That is four values this
repository configured and one artefact it built, read back out of the index rather than asserted
from inside the run that uploaded them.

**THE WITNESS RECORD WAS BUILT OUTSIDE THE PIPELINE UNDER A RULING, AND THIS IS THE FIRST ENTRY THAT
HAS TO SAY SO.** The `witness` job of run 34687815710 is the only job of the six that failed — it ran
10:49:36–10:49:47Z and is still `failure` today, which is a fact about the run and not a thing this
entry repairs. It failed because `build_witness.py` read the approval instant from
`actions/runs/<id>/approvals`, and that payload carries no timestamp at any level: its one entry's
keys are exactly `comment`, `environments`, `state` and `user`. So the instant came out empty,
`gates/witness_verify.py` refused the record it was given, and `witness-2.1.2.json` was neither built
nor attached. **The repair is commit `ac684d8d`, and it is a separate commit from this one**: the
builder now takes `approved_at` from the `queued` status of the deployment whose statuses name the
run and whose environment matches the approval's. The 2.1.2 record was then produced by running
**that** builder — the same code the next release's pipeline will run — against run 34687815710's own
inputs, and it was accepted by `gates/witness_verify.py --download` before it was attached to the
Release or committed here. **No tag was made, moved or re-run to obtain it.** The alternative would
have been a fourth tag for a release that had already succeeded, and the ruling that authorised the
out-of-pipeline build is the one that refused that: a record that cannot be verified is not
committed, and a missing instant is a refusal rather than a field to fill in by hand.

**`releases/witness/2.1.2.json` and the Release asset are the same bytes**, sha256
`f20b86a42692aef0e0b79037f0b430336530306438fc1938bb89b6793cd94cd9` on both, established by
downloading the attached asset fresh and comparing it to the committed file rather than by trusting
the upload. `approved_at` in it reads **2026-09-12T10:48:52Z**, equal to the deployment's `queued`
status above; `review_file` reads `rounds/reports/PR.review.md`, which is the verdict the approval
...
the architecture set for `evidence.available` — release evidence generated from the release commit,
attached, referenced by the witness record and retrievable by a third party — is therefore satisfied.
**The field is nevertheless still `false` in all fourteen adapter manifests, and it will stay false in
the published 2.1.2 distribution permanently**, because that distribution was built and uploaded
before the flip could be considered. The flip is a fourteen-module code change with a bump
consequence, it does not belong in the commit that records a release, and it was deferred on
2026-09-12 to a round of its own rather than smuggled into this one. This paragraph exists so that a
reader who installs 2.1.2, reads `available: false` and finds the evidence bundle hanging off the
Release is not left to guess which of the two is wrong: neither is, and the manifests are behind.

**The alert set, re-read on the day of the witness rather than carried from the release.** At
**2026-09-12T18:16:57Z** the repository had **three** open Dependabot alerts, all against
`docs/package-lock.json`: `image-size` **high** twice (`GHSA-w3rx-r6r6-pgpr`, `GHSA-5p2g-fcmc-qvqq`)
and `uuid` **medium** (`GHSA-w5hq-g745-h8pq`). **No CRITICAL, and no HIGH outside the two approved
exceptions.** None of the three is in the distribution: the lock file is the documentation site's,
and neither `synapse_cdm-2.1.2-py3-none-any.whl` nor its sdist contains it.

**THE DOCUMENTATION SITE WAS TWO RELEASES BEHIND AND THE WITNESS READ IT BEFORE THE UPLOAD, WHICH IS
NOW THE ORDINARY SHAPE.** Before the deploy, `gates/deploy_record.py` read the served changelog page
at **18:19:46Z** stating `2.0.0` against a tree declaring `2.1.2`. The site was rebuilt from the clean
tree **at the tag** — a detached checkout of `5ab80f5f`, porcelain 0 — so the stamp is the tag's and
not this round's. Two consecutive builds from that tree are **byte-identical across all sixty-six
files**, and the page the deploy carries is 66 679 bytes,
`d02dc4dd83bafc74f7819b06c7db8fba5a2aca42ada57d9f157dce39d7499d05`. **A third build, taken first and
from a cold module cache, differed** — in one file, the 4 854-byte webpack runtime chunk, whose
minified local names were assigned in a different order, which moves that file's content hash and
therefore every page's reference to it. The bytes deployed are the ones two builds agreed on.

**Deployment `87f0989f`, source `5ab80f5f`, uploaded 18:22:22–18:22:37Z** — 49 of 66 files new, 17
already held, wrangler 4.131.1, preview `https://87f0989f.synapsecommand-docs.pages.dev`. The upload
instants are the client's clock and are said to be so; Cloudflare's own list reports ages rather than
instants. **The alias served the new page eight seconds after the upload returned**: at **18:22:45Z**
`https://docs.synapsecommand.com/changelog/` was byte-identical to the local build, and so was the
deployment's own `pages.dev` URL. **The gate refused once and then reconciled**, which is the record
working: at 18:25:56–18:26:02Z it named `7031ab1f` against bytes that said `87f0989f` — the ninth time
it has caught its own round's upload — and after the row, the count and the alias paragraph of ledger
entry 8 moved in one edit it read **24 listed; 13 with a row, 11 covered retrospectively; 0
unaccounted for**, alias `87f0989f`, **2 checks, 0 failed** at 18:26:39–18:26:44Z.

**THE SERVED-VERSION WITNESS STILL READS DISAGREE AFTER THE DEPLOY, AND THE CAUSE IS THE PARSER
RATHER THAN THE SITE — A FINDING, RECORDED AND NOT REPAIRED HERE.** At 18:26:44Z it read `states
2.0.0 and version.py declares 2.1.2 — DISAGREE` against the page this round had just deployed. The
page is not stale: it carries **five** sentences of the form `package is at <code>N.N.N</code>`, in
append order `2.0.0, 2.0.0, 2.1.0, 2.1.1, 2.1.2`, one per dated paragraph of the versioning note, and
the newest is **last**. `gates/deploy_record.py` takes the **first** match, so on a page whose
convention is that a dated reading is appended and never edited, the witness is pinned to the oldest
reading it can find. It read AGREE at ledger entries 15 and 18 only because the newest sentence
happened to be the first one then. The witness cannot fail the gate by design, so nothing was blocked
by this; the check is named here rather than corrected, because a gate is not repaired in passing by
...
present-tense sentence about what the index serves stopped being true at 10:49:11Z; and
`packages/cdm/synapse_cdm/MIGRATIONS.md` records the act. `tests/test_cdm_publication.py` moves its
docstring and its number vocabulary, which ended at eighteen and the entry this round adds is the
nineteenth — the same two lines the 2.0.0 witness round had to move, for the same reason. **Nothing
under `packages/cdm/synapse_cdm/` moved except `MIGRATIONS.md`**: `version.py`, `RELEASE_NOTES.md`,
`klv_pin.json` and `FORMAT_COVERAGE.md` are byte-identical to `5ab80f5f` at the close, verified by
`git ls-tree` rather than by intent, and `PACKAGE_VERSION` still reads `2.1.2`.

**What this entry does not claim.** It does not claim the `witness` job works — it failed on this run
and the repair has never been exercised by a tag push; the next release is what will witness that, and
this record is the same code's output over the same inputs rather than a run of the pipeline. It does
not claim a digest for the attestation bundle: `attestation.bundle_sha256` in the record is the **empty
string**, because the pipeline passes the verification's instant and not the bundle, and the verifier
checks the field is present rather than re-establishing it. It does not claim the two refused tags
were the only way to find either defect, only that they are where each was found. It does not claim
`evidence.available` is true anywhere. It does not claim the documentation site is gated: the
served-version witness reports and cannot fail, and this entry records it reporting a disagreement it
cannot resolve. It does not claim the simple index's lag has a length. It does not claim PyPI's
`provenance` key is absent on any release but 2.1.2 today. And it does not claim that a reviewing
session is a person: the twenty minutes and forty-three seconds this entry measures as a hold are a
hold on a verdict, as entries 16, 17 and 18 measure theirs.

### 20. `synapse-cdm` 2.2.0 is on the index — CLOSED, and it is the first release since 2.1.0 whose number moved for what the distribution carries rather than for what a workflow refused

**Published 2026-09-17** by
[run 35200069387](https://github.com/Decent-Cybersecurity/synapsecommand-public/actions/runs/35200069387),
triggered by the `v2.2.0` tag at `5c53e756` — tag object `55b717dd`, annotated, tagger Matej Michalko,
created **07:37:15Z**, the same second as the commit it names. The run was created at 08:30:50Z by the
push, fifty-three minutes and thirty-five seconds after the tag object was written, and that gap is the
release procedure rather than a delay: the tag is made locally, the branch is pushed alone, the commit's
code-scanning analyses are waited for and read, the ref rehearsal is run against them, and only then are
`main` and the tag pushed together. The gate job ran 08:30:54–08:36:04Z, the build job 08:36:07–08:44:19Z
and the attestation job 08:44:22–08:44:51Z; the `pypi` environment then held the upload for **six hours,
twenty minutes and thirty-one seconds** until it was approved at **15:05:23Z**, and the publish job ran
15:05:25–15:05:45Z, twenty seconds. Deployment `6498957653` carries the sequence: `waiting` at 08:44:52Z,
`queued` at 15:05:23Z, `in_progress` at 15:05:25Z, `success` at 15:05:45Z. **The second act, tag push to
`success`, took six hours, thirty-four minutes and fifty-five seconds**, of which the hold was six hours,
twenty minutes and thirty-one. The `release` job then ran 15:05:49–15:06:14Z and published the Release at
15:06:11Z, as the same job did for 2.1.2; the pipeline has six jobs and five of them succeeded.

**THE NUMBER IS THE DERIVED FLOOR, AND THIS ENTRY'S FIRST SUBJECT IS THAT NOTHING WAS REFUSED TO
REACH IT.** `v2.2.0` is the first and only tag of this release: no earlier tag names a commit the index
has never seen, and the "TWO EARLIER TAGS" paragraph entry 19 had to write has no counterpart here.
`gates/bump_derivation.py` over the arc `v2.1.2 -> v2.2.0` derives **MINOR** from the public names the
arc added — eighty-three units the table could not decide, every one ruled in `MIGRATIONS.md`'s 2.2.0
section, and none unruled — so the floor and the number are one number and no version ruling exists
for it. Read out of the downloaded wheel rather than out of this tree, `synapse_cdm/version.py` line 326
reads `PACKAGE_VERSION = "2.2.0"`, line 285 `SCHEMA_VERSION = "2.1.0"` and line 343
`ADAPTER_API_VERSION = "2.1.0"`: the package moved a MINOR, the wire contract did not move, and the
Adapter API moved 2.0.0 -> 2.1.0 on its own row. The tag message's own claim — "the first number
since 2.1.0 that moved for what the distribution carries rather than for what a workflow refused" —
is this entry's heading because it is what the readings above say.

**The digests, and all six were reachable from the Release alone.** `SHA256SUMS` names
`e0e87ff1bb5646e8d7503ce26d7bd55973d5f872a99f6b8f3f8b1ce25b42de23` for the wheel,
`4a2c79fde212dcc172f73ccea5933e23e3c81ff5a7a5e4d333e786c1870ca82a` for the sdist,
`af4de5f3a5f4ccbb6401aa143130e6a556946b2f357c3af9e2636d6daec557be` and
`759d2272e80a85f7e4ba36cc1372534b08665ba645ad887815e673099f53337a` for the CycloneDX and SPDX
SBOMs, `01131690c7ac046eaac3ef8c881c22c5eb2c19aaf577ac398495ca1354422480` for the conformance sweep
and `c28bc7cf468b4847fca8dcac82a26f4f16797a7ce9471380da9136c6800afa44` for the evidence bundle.
**The index agrees about the two that reach it**: `GET /pypi/synapse-cdm/2.2.0/json` returns the same
wheel and sdist digests, with sizes 7 220 819 and 5 176 196 bytes and upload times
**15:05:40.188470Z** and **15:05:41.907530Z** — one second and seven hundred and nineteen milliseconds
apart, and the second of them is the instant this release became a thing a stranger could install.
Both files were downloaded from `files.pythonhosted.org` at 15:12:51Z, their archive magic checked
first, and re-hashed to exactly those digests at those sizes. The Release body is byte-identical to the
`release-notes-2.2.0.md` asset, `1b48d07165db5a10e79759c59a900571bb9096467ca900084b62b5590a4911d3`
over 45 169 bytes on both.

**THE PyPI PROVENANCE TRAP, ELEVENTH READING, AND IT READS AS THE FIFTH THROUGH TENTH DID.** Read
2026-09-17T15:38:14Z, the two per-file objects for 2.2.0 in `GET /pypi/synapse-cdm/2.2.0/json` carry no
`provenance` key at all — sixteen keys each, probed for the KEY and not for a value, because the
endpoint stopped carrying it rather than started answering `null`. **The attestations are on the simple
index, and they witness the publisher from PyPI's own side.** Read 15:43:14Z, `GET /simple/synapse-cdm/`
with `Accept: application/vnd.pypi.simple.v1+json` lists fourteen versions and twenty-eight files, the
last two 2.2.0's, and gives each of them a `provenance` URL —
`https://pypi.org/integrity/synapse-cdm/2.2.0/<filename>/provenance`, which entry 19 did not quote; both
answer 200 with one bundle holding one attestation. The bundle at the wheel's names `kind` **GitHub**,
...
with `subject[0].digest.sha256` `e0e87ff1…4de23`, the wheel's own digest. That is four values this
repository configured and one artefact it built, read back out of the index rather than asserted from
inside the run that uploaded them.

**THE WITNESS JOB EXECUTED THE REPAIRED BUILDER FOR THE FIRST TIME, BUILT THE RECORD, AND WAS REFUSED
BY ITS OWN VERIFIER ON AN EMPTY APPROVAL COMMENT — AND THIS IS THE SECOND ENTRY THAT HAS TO SAY THE
RECORD WAS BUILT OUTSIDE THE PIPELINE.** The `witness` job of run 35200069387 is the only job of the six
that failed — it ran 15:06:19–15:06:26Z and is still `failure` today, which is a fact about the run and
not a thing this entry repairs. Its build step succeeded, which is what entry 19 said the next tag push
would be the first to show: under the `actions: read` and `deployments: read` grants of 2026-09-16 it
read the run's approvals and the status history of `5c53e756`'s one deployment, fetched the attestation
store by the wheel's digest, handed the builder `--deployment-statuses`, `--run-id` and
`--attestation-bundles`, and at 15:06:25Z the log reads `wrote witness-2.2.0.json for v2.2.0 (2 files,
1 approval(s))` — the round-PW repair produced the instant, and the 2026-09-16 additions produced the
bundle digest, on a tag for the first time. The verify step, `gates/witness_verify.py witness-2.2.0.json
--download --assets assets` with a token, then refused it, and its whole output is four lines:

```
  DISAGREES   witness-2.2.0.json (1)
    - an approval entry has neither a `review_file` nor a `comment`, so nothing says what it was taken on
1 witness record(s), 1 disagreeing
##[error]Process completed with exit code 1.
```

The approval that released the hold at 15:05:23Z — `state` **approved**, `user` **decentcybersecurity**,
`environments[0].name` **pypi**, and a `comment` that is the empty string, `""`, read from
`actions/runs/35200069387/approvals` on the day of the release and again at 15:38:14Z today — is
exactly the input the builder's own test module says must be refused: the maintainer approved with no
comment, the builder wrote `""` into both `comment` and `review_file` as it is required to, and the
verifier's rule of 2026-09-16 (`gates/witness_verify.py` lines 203–205: neither field filled is an
approval nobody can trace) is the one that held. It is not the defect entry 19 recorded — the builder
is not wrong and there is no code repair to name — it is a human input the rule was written for. The
"Attach it to the Release" step was `skipped`; the Release has eight assets and no `witness-2.2.0.json`,
and the bytes the job built are not retrievable from anywhere, because the attach step is also the one
that prints them into the run summary. **The 2.2.0 record was therefore hand-built by this witness
round, under the ruling entry 19 records and the procedure `MIGRATIONS.md`'s release sequence lays down
for the step after the run, which says a round doing so states so here**, with the same builder over
the run's own inputs — the Release payload,
the approvals payload, the deployment statuses and the attestation store's answer, all re-fetched from
the API at 15:37:26Z — and one argument the pipeline never passes. No tag was made, moved or re-run to
obtain it.

**THE MAINTAINER'S DESIGNATION, AND WHAT THE RECORD SAYS ABOUT IT.** Rebuilt without that argument, the
record is the pipeline's own refused shape: `review_file` empty, `comment` empty, sha256
`e9dc2abe8c8f2f5f2d9a764dba19337a9d6adc80fd78383d6ed5c34879b067e7` over 1 844 bytes, and
`gates/witness_verify.py` refuses it in every mode with the same complaint the job printed. Matej
Michalko ruled on 2026-09-17, in the witness round, that the record's one approval entry carries
`review_file` =
...
— the release-readiness report at the release commit, the document the release-readiness protocol says
an approval is taken on, and the document commit `12ca67e3` re-qualified for 2.2.0 at 07:14:21Z on the
day of the release, twenty-two minutes and fifty-four seconds before the tag object was written — **as
a reference designated by the maintainer in the witness round**,
and that `comment` stays `""` as recorded. This entry states plainly what that means: the approval's
comment was empty; the pipeline's verify step refused the record for that reason; and the reference the
committed record carries was designated by the maintainer afterwards rather than named by the approval.
The designation is a builder argument and not an edit: `.github/scripts/build_witness.py` gains
`--review-file URL` in this commit — accepted only as an `https://` URL, applied only to an approval
whose comment names no URL, refused when the comment names a different one, and never passed by
`publish.yml` — so the committed record is reproducible as one command, the job's own command line plus
that flag, and `tests/test_cdm_witness_builder.py` holds each of those four properties. The builder
without the flag still writes `""` for an empty comment, and the test that says so is unchanged.

**`releases/witness/2.2.0.json` is that command's output**, sha256
`798daa89bff5fc2332d56e010a4b5837f70fe4abe21b05e1e3c4afb253d7582e` over 1 988 bytes, and it differs
from the refused shape in exactly one line, the eighth, `review_file`. It was accepted by
`gates/witness_verify.py` before it was committed, in every mode the verifier has — `--offline --assets`
over a fresh `gh release download` of the Release, plain online against the index and the Release, and
`--download --assets` with a token, the job's own invocation, which re-hashed the served bytes of all
six assets and read the wheel's bundle back from the attestation store — and again over the committed
path after the copy. **It is on no Release**: entry 19's record and its Release asset were the same
bytes, and this one has no asset to be the same bytes as, because the job that would have attached it
was refused; attaching the committed bytes afterwards would be a further act with `contents: write`, and
it is not this round's. `approved_at` in it reads **2026-09-17T15:05:23Z**, equal to the deployment's
`queued` status above; `attestation.verified_at` reads **2026-09-17T08:44:50Z**, the attest job's own
step 6; and `attestation.bundle_sha256` reads
`9d78a5205dcfcf5967faba5357c3e7f6ff7ea020dabe8b6ebde46083bd4436a1`, the canonical digest of the one
bundle the store serves for the wheel's digest (`repository_id` 1343031092) — the first record in this
directory to carry one, where 2.1.2's carries the empty string. It is the **second** record in
`releases/witness/`, and the second built by hand.

**THE EVIDENCE IS ATTACHED AND RETRIEVABLE AND THE FOURTEEN MANIFESTS NOW SAY SO — ENTRY 19's
DEFERRED GAP CLOSES IN THIS DISTRIBUTION.** `evidence-2.2.0.tar.gz` is the Release's own asset,
141 564 bytes, digest above, downloadable by anybody; the witness record references it. Commit
`f36550d` (2026-09-12, round PE) flipped `evidence.available` in all fourteen adapters and is an
ancestor of `v2.2.0`; read out of the downloaded wheel, `synapse_cdm/adapters/*.py` carries
`Evidence(available=True)` fourteen times and `available=False` never. 2.1.2 remains, permanently,
the distribution that says `false` beside a bundle that exists, as entry 19 said it would.

**The alert set, re-read on the day of the witness rather than carried from the release.** At
**2026-09-17T15:38:14Z** the repository had **three** open Dependabot alerts, all against
`docs/package-lock.json`: `image-size` **high** twice (`GHSA-w3rx-r6r6-pgpr`, `GHSA-5p2g-fcmc-qvqq`)
and `uuid` **medium** (`GHSA-w5hq-g745-h8pq`) — the same three as entry 19, and every one of them
`updated_at` 2026-09-08. Entry 19's "no HIGH outside the two approved exceptions" cannot be repeated,
because `security/exceptions/` holds no exception file today: the audit of 2026-09-16 deleted both
`image-size` files on the trigger they named, and the committed lock resolves `image-size` to 2.0.4,
outside the `<= 2.0.2` range both advisories state — a reading of Dependabot's own state beside the
lock's, recorded here and not resolved here. None of the three is in the distribution: the lock file is
the documentation site's, and neither `synapse_cdm-2.2.0-py3-none-any.whl` nor its sdist contains it.

**THE DOCUMENTATION SITE IS ONE RELEASE BEHIND AT THE TIME OF WRITING, AND NO DEPLOY IS PART OF THIS
COMMIT.** At **2026-09-17T15:38:14Z** `https://docs.synapsecommand.com/changelog/` answered 200 with
66 679 bytes, `d02dc4dd83bafc74f7819b06c7db8fba5a2aca42ada57d9f157dce39d7499d05` — byte-identical to the
page entry 19 recorded deployed as `87f0989f`. It carries five sentences of the form
`package is at <code>N.N.N</code>`, in append order `2.0.0, 2.0.0, 2.1.0, 2.1.1, 2.1.2`, and
`gates/deploy_record.py`'s `served_version`, which since 2026-09-16 reads the last of them, reads
`2.1.2` against a tree declaring `2.2.0`: the witness reads `DISAGREE` for the ordinary reason entry 19
named as the ordinary shape, the site not yet rebuilt from the tagged tree. The tree's own changelog
page carries a sixth sentence, `2.2.0`, so a deploy from `5c53e756` would read `AGREE`; that deploy,
entry 8's row for it, and its reconcile are a later act and none of this commit.

**THE DEPLOY, 2026-09-17 — THE LATER ACT THE PARAGRAPH ABOVE NAMED, AND THIS IS ITS RECORD, WRITTEN
IN THE COMMIT AFTER THIS ENTRY'S.** Before the deploy, `gates/deploy_record.py` read the served
changelog page at **17:39:29Z** stating `2.1.2` against a tree declaring `2.2.0`, with twenty-four
deployments listed and all accounted for and the alias at `87f0989f`. The site was rebuilt from the
clean tree **at the tag** — a detached checkout of `5c53e756`, porcelain 0 — so the stamp is the
tag's and not the witness round's; the tree at `bbe5140` carries two documentation pages the witness
round rewrote, and they are served by the next deploy and not this one, as entry 19's deploy served
the tagged tree and not its own witness commit. Two consecutive builds from that tree are
**byte-identical across all sixty-six files**, and the page the deploy carries is 68 835 bytes,
`501d1f146b12565aa987378218f109e84dda1e1e51231e5467787888a956c100`, six sentences of the form
`package is at <code>N.N.N</code>` in append order `2.0.0, 2.0.0, 2.1.0, 2.1.1, 2.1.2, 2.2.0`.

**Deployment `9f0e9022`, source `5c53e756`, uploaded 17:40:02–17:40:14Z** — 43 of 66 files new, 23
already held, wrangler 4.134.0, preview `https://9f0e9022.synapsecommand-docs.pages.dev`, the
deployment stamped with the tag's commit and `--commit-dirty=false` because the built bytes are the
tag's and the working tree that ran wrangler was one commit past it. The upload instants are the
client's clock and are said to be so. **The alias served the new page six seconds after the upload
returned**: at **17:40:20Z** `https://docs.synapsecommand.com/changelog/` was byte-identical to the
local build, and so was the deployment's own `pages.dev` URL; one read earlier, at 17:40:14Z, the
domain still answered with `87f0989f`'s page. **The gate refused once and then reconciled**, which is
the record working: at 17:40:20Z it named `9f0e9022` as the one deployment the table could not name
— the tenth time it has caught its own round's upload — and after the row, the count and the alias
paragraph of ledger entry 8 moved in one edit it read **25 listed; 14 with a row, 11 covered retrospectively; 0 unaccounted for**, alias `9f0e9022` (identical on 5 of five pages, differing from `87f0989f` on 5), **2 checks, 0 failed** at 17:49:28Z–17:49:37Z. The served-version witness,
which since 2026-09-16 reads the last `package is at` sentence, reads `states 2.2.0 and version.py declares 2.2.0 — AGREE` at 17:49:45Z. This act
writes one file, `PUBLICATION.md`, and nothing under `packages/cdm/`; `PACKAGE_VERSION` still reads
`2.2.0`.

**THE SECOND DEPLOY OF THE DAY, 20:04Z — `main`'s tip, so the site says what the tree says.** The
paragraph above deployed the tagged tree and named the pages it left unserved; by the time of this
act `main` had taken three more commits — the witness round (`bbe5140`), the docs record
(`0064b35`) and the npm record (`98896bf`) — and three documentation pages differed from the tag:
the changelog's dated correction on what the index serves, the release-pipeline page's account of
run 35200069387, and the supply-chain page's seven overrides. Before the deploy the gate read, at
**20:03:53Z**, twenty-five listed and all accounted for, alias `9f0e9022`, served `2.2.0` — AGREE.
The site was rebuilt from the clean tree at `98896bfc`, porcelain 0, `main` and `soif/1.0` at the
same commit; two consecutive builds byte-identical across all sixty-six files, thirty-six of them
differing from the deployed tag build; the changelog page 69 834 bytes,
`18e9f1d0a6d874b748e8bba07199502edb545ab936f6ffe0aff506743af955f2`, the same six `package is at`
sentences ending `2.2.0`. **Deployment `880bf67c`, source `98896bfc`, uploaded 20:04:12–20:04:24Z**
— 31 of 66 files new, 35 already held, wrangler 4.134.0, preview
`https://880bf67c.synapsecommand-docs.pages.dev`; the alias served the new page **six seconds** after
the upload returned, at **20:04:30Z**, byte-identical to the local build and to the deployment's own
`pages.dev` URL. The gate refused once, at 20:04:31Z, naming `880bf67c` as the one deployment the
table could not name — the eleventh time — and after the row, the count and the alias paragraph of
entry 8 moved it read — after a second refusal, of a record that was right, described next —
**25 listed — the newest 25, which is all wrangler returns; 15 with a row, 11 covered retrospectively; 0 unaccounted for; 1 recorded beyond the window, `039866b1`**, alias `880bf67c` (identical on 5 of five pages, differing from `9f0e9022` on 5), **2 checks, 0 failed** at 20:08:15Z–20:08:23Z; the served-version witness reads `states 2.2.0 and version.py declares 2.2.0 — AGREE` at 20:08:31Z.
This act writes `PUBLICATION.md`, and — for the reason the next paragraph gives — one gate and its
test module.

**THE GATE REFUSED A SECOND TIME, AT 20:05:23Z, ON A RECORD THAT WAS RIGHT, AND THE CAUSE IS A
WINDOW THE TWENTY-SIXTH DEPLOYMENT OPENED.** With the row written, direction two of
`gates/deploy_record.py` — the one that stops the record being satisfiable by typing rows — named
`039866b1` as an id Cloudflare does not list. It is the project's first deployment, of 2026-08-22,
one of the eleven the coverage set names, and Cloudflare still holds it: `wrangler pages deployment
list` returns the newest **twenty-five** deployments and offers no page, so the twenty-sixth pushed
the first out of the list, and a gate that had read the list as the whole history read a true record
as an invented one. The pre-deploy reading at 20:03:53Z passed only because the list was exactly
twenty-five then. **Repaired in this act, against the rule entry 19 applied to the served-version
witness, and the difference is stated:** that witness reports and cannot fail, so its defect could
wait for a ruling; this check fails, and with the list unbounded it refuses every honest record from
the twenty-sixth deployment on, so a reconciliation that cannot pass reconciles nothing. The repair
is the smallest one that keeps the check honest: a list shorter than the window is the whole history
and a recorded id it lacks is refused as before; a list that is full is read as a window, and a
recorded id beyond it is measured and named — `1 recorded beyond the window: 039866b1` — never
refused. The bound is real and the gate's docstring says so: a deployment deleted from beyond the
window is a deletion this gate cannot see. `tests/test_cdm_deploy_record.py` moves its direction-two
fixture to a list shorter than the window and gains the window case, a full list with the oldest
recorded id absent passing with that id named, and a fabricated unrecorded id in the same full list
still refused.

**This round writes ten files**, and the eleventh is the test module that states the ledger count.
`releases/witness/2.2.0.json` is new; `PUBLICATION.md` gains this entry; `docs/docs/changelog.mdx`
gains a dated correction beside the 2.2.0 paragraph whose present-tense sentence about what the index
serves stopped being true at 15:05:41Z; `packages/cdm/synapse_cdm/MIGRATIONS.md` records the act, opens
the pending section this one-file change makes necessary, and rewrites its three dated witness
paragraphs of 2026-09-16 to say what run 35200069387 did; `releases/witness/README.md` and
`docs/docs/security/release-pipeline.mdx` are rewritten the same way, and the README says what
`review_file` means for 2.2.0; `SECURITY.md`'s SBOM row gains the v2.2.0 asset sizes;
`.github/scripts/build_witness.py` gains `--review-file`, `tests/test_cdm_witness_builder.py` its
tests, and `tests/test_cdm_witness.py` moves the assertion that held the directory to one record.
`tests/test_cdm_publication.py` moves its docstring and its number vocabulary, which ended at nineteen.
**Nothing under `packages/cdm/synapse_cdm/` moved except `MIGRATIONS.md`**, verified by `git ls-tree`
against `5c53e756` rather than by intent, and `PACKAGE_VERSION` still reads `2.2.0`.

**What this entry does not claim.** It does not claim the `witness` job works end to end: its builder
ran on a tag for the first time and produced a record, its verifier refused that record on the approval
it was given, and no run has yet attached one. It does not claim the approval was taken on the readiness
report: the comment is the empty string, that is what the record carries, and the reference beside it is
the maintainer's designation after the fact, which this entry says in as many words. It does not claim
the committed record is the pipeline's record, only that it is the same builder's output over the same
inputs plus one designated argument, and that it differs from the refused shape in the one line it says.
It does not claim `witness-2.2.0.json` is on the Release. It does not claim the documentation site is
gated, or deployed: the served-version witness reports and cannot fail, and this entry records it
reporting a disagreement this commit does not resolve. It does not claim the two open `image-size`
alerts are false positives, only what the lock resolves and what the advisories state. It does not claim
the simple index's lag has a length. It does not claim PyPI's `provenance` key is absent on any release
but 2.2.0 and 2.1.2 today. And it does not claim that the hold was a hold on a verdict: entries 16
through 19 could say so because their approval comments named one, and this one named nothing — the
six hours, twenty minutes and thirty-one seconds this entry measures are a hold released by an approval
whose comment is empty, and the record says so rather than borrowing the earlier entries' sentence.

### 21. `synapse-cdm` 3.0.1 is on the index — CLOSED, and it is the first release of the audit remediation arc, the corrective of a tag burned the same day below the gate, and the first whose witness record the pipeline produced

**Published 2026-09-20** by
[run 35514833652](https://github.com/Decent-Cybersecurity/synapsecommand-public/actions/runs/35514833652),
triggered by the `v3.0.1` tag at `89d2c707` — tag object `fa11e55e`, annotated, tagger Matej Michalko,
created **13:47:50Z**. The run was created at 13:53:41Z by the push, five minutes and fifty-one seconds
after the tag object was written, and that gap is the release procedure: the tag was made locally after
`main` had been pushed alone and the commit's two CodeQL analyses had been waited for, the ref rehearsal
was run against them — seven checks, none failed — and the six quick-run modules the tag turns green were
run at the tag, and only then was the tag pushed. The gate job ran 13:53:44–14:01:41Z, the build job
14:01:45–14:18:42Z and the attestation job 14:18:46–14:19:08Z; the `pypi` environment then held the
upload for **twenty-four minutes and one second** until it was approved at **14:43:10Z**, and the publish
job ran 14:43:11–14:43:38Z, twenty-seven seconds. Deployment `6553754047` carries the sequence: `waiting`
at 14:19:09Z, `queued` at 14:43:10Z, `in_progress` at 14:43:12Z, `success` at 14:43:39Z. **The second act,
tag push to `success`, took forty-nine minutes and fifty-eight seconds**, of which the hold was
twenty-four minutes and one. The `release` job ran 14:43:41–14:44:05Z and published the Release at
14:44:02Z, and the `witness` job ran 14:44:08–14:44:27Z; the pipeline has six jobs and **all six
succeeded**, which no tag push had produced before.

**THIS IS A CORRECTIVE, AND ITS FIRST SUBJECT IS THE TAG IT CORRECTS.** `v3.0.0` was tagged on
`ca445c6` earlier the same day and pushed, and [run 35506445471](https://github.com/Decent-Cybersecurity/synapsecommand-public/actions/runs/35506445471)
refused it — not in the gate job, where `v2.1.0` and `v2.1.1` died, but in the build job at condition 4,
the second run of the suite. The gate had run the same suite on the same commit and read `5895 passed,
86 skipped in 810.75s`; the build job's run read `1 failed, 5894 passed, 86 skipped in 1262.27s`, and the
step piped pytest into `tail -1`, so the run recorded a count and no name. The cause was reconstructed
rather than read: the build job's install step had put `twine` and `cyclonedx-bom` into the interpreter
that then ran the suite, `cyclonedx-bom` brings `jsonschema[format]`, `jsonschema` imports those eight
format libraries at import time — `rfc3987-syntax` builds a Lark grammar on import — and
`synapse_cdm.suite` reaches `jsonschema` through `harness` and `schemas`, so every parser worker the
isolation, envelope and conformance tests spawn paid for it before its first byte of input. Measured on
the maintainer's machine with exactly those additions, `import synapse_cdm.suite` went from 0.11 s to
0.48 s and the three worker-spawning test modules from 9.9 s to 19.0 s; the wall-clock-budgeted
isolation tests run at 2.5 s there against a 10 s bound, and the build runner ran the suite 4.3 times
slower than that machine. Nothing about the ref was involved, so `gates/release_ref_rehearsal.py`
passed `v3.0.0` and was right to. `MIGRATIONS.md`'s 3.0.1 section is the record: the release tooling now
lives in a venv of its own (`${TOOLS}`), condition 4 writes the suite's output to a file and prints its
`FAILED` and `ERROR` lines before it stops, `tests/test_cdm_trusted_publishing.py` holds both, and no test
budget moved. `v3.0.0` stays where it is, permanently, the third tag in this repository's history that
names a commit the index has never seen; nothing reached PyPI from it, no `pypi` hold was created and
no GitHub Release exists for it.

**THE NUMBER IS THE DERIVED FLOOR, TWICE OVER.** `gates/bump_derivation.py` over the arc `v3.0.0 ->
v3.0.1` derives **PATCH** from one signal, `synapse_cdm/MIGRATIONS.md` on the shipped-document row, with
nothing unruled and no version ruling; the arc moved the release workflow, three test modules, the
frozen-contract manifest's provenance, the release notes and the version sites, and nothing importable.
Over what the index served before this upload — 2.2.0 — the release is the MAJOR the 3.0.0 release
commit typed on both first axes, and the 3.0.0 section and release notes describe it; a reader upgrading
from 2.2.0 gets that arc and this corrective together, because 3.0.0 was never installable. Read out of
the downloaded wheel rather than out of this tree, `synapse_cdm/version.py` line 398 reads
`PACKAGE_VERSION = "3.0.1"`, line 336 `SCHEMA_VERSION = "3.0.0"`, line 425 `ADAPTER_API_VERSION = "3.0.0"`,
line 478 `MANIFEST_SCHEMA_VERSION = "2.1.0"`, line 500 `EVIDENCE_SCHEMA_VERSION = "2.0.0"` and line 404
`SC_OES_VERSION = "0.1.0"`: the package is one PATCH past the wire contract, for a workflow's refusal and
not for a contract decision, which is the third time in this file's ledger that has been the reason.

**The digests, and all six were reachable from the Release alone.** `SHA256SUMS` names
`cc70c59ec6ed97f470c5b9140d7b37d81633ca06cc6750802d504e0ec29f486a` for the wheel,
`a8aa0a0e75d0065f7bdcda4ab04885610e1aabd856e4732186e2029856e719c4` for the sdist,
`2cfb30882f4ea75cbffdd30af86e526286be1f0dce45ecc0eb8eec3f67ef27ea` and
`6cee32b1f15f90b9deb3d833215d1e75ab9ef94ffbaecae64eea42f6e0124214` for the CycloneDX and SPDX SBOMs,
`717d377905212240308d7676a841a4cfa76549cadc69cb39d465baa45deee6d8` for the conformance sweep and
`05f6adb9de249d5842a4305fdd4f245e5f7d1d51e500f59556ad26be7253c0e9` for the evidence bundle; the
Release API reports the same six digests on the same six assets, at 7 313 809, 5 268 719, 69 254,
110 994, 290 231 and 160 349 bytes. **The index agrees about the two that reach it**:
`GET /pypi/synapse-cdm/3.0.1/json` returns the same wheel and sdist digests, with sizes 7 313 809 and
5 268 719 bytes and upload times **14:43:33.200516Z** and **14:43:35.275539Z** — two seconds and
seventy-five milliseconds apart, and the second of them is the instant this release became a thing a
stranger could install. Both files were downloaded from the index at 15:04:00Z, their archive magic
checked first (`PK\x03\x04`, `\x1f\x8b`), and re-hashed to exactly those digests at those sizes. The
Release body is byte-identical to the `release-notes-3.0.1.md` asset,
`5725b6ed0a2ea4a0099ed446d31210014f5f5157ccfa5cca6bd8c07c98d124bc` over 45 384 bytes on both, once the
one trailing newline the API's JSON rendering appends is set aside.

**THE PyPI PROVENANCE TRAP, TWELFTH READING, AND IT READS AS THE FIFTH THROUGH ELEVENTH DID.** Read
2026-09-20 in the witness round, the two per-file objects for 3.0.1 in `GET /pypi/synapse-cdm/3.0.1/json`
carry no `provenance` key at all — sixteen keys each, probed for the KEY and not for a value. **The
attestations are on the simple index, and they witness the publisher from PyPI's own side.**
`GET /simple/synapse-cdm/` with `Accept: application/vnd.pypi.simple.v1+json` lists fifteen versions and
thirty files, the last two 3.0.1's, and gives each of them a `provenance` URL under
`https://pypi.org/integrity/synapse-cdm/3.0.1/<filename>/provenance`; both answer with one bundle holding
one attestation. The bundle at the wheel's names `kind` **GitHub**, `repository`
**Decent-Cybersecurity/synapsecommand-public**, `workflow` **publish.yml**, `environment` **pypi**, and its
in-toto statement's `predicateType` is `https://docs.pypi.org/attestations/publish/v1` with
`subject[0].digest.sha256` `cc70c59e…9f486a`, the wheel's own digest; the sdist's bundle reads the same
four values with `a8aa0a0e…e719c4`. Four values this repository configured and two artefacts it built,
read back out of the index rather than asserted from inside the run that uploaded them. The
repository's own attestation job had verified the wheel, the sdist, both SBOMs and the evidence bundle
at 14:18:50–14:19:07Z, before the hold, and the witness record carries that instant and the bundle's
canonical digest, `921e2fd10f80509bd17c89f2cd553bb5ef41526c605d67c98383e2160f7d124c`.

**THE WITNESS JOB SUCCEEDED END TO END FOR THE FIRST TIME, AND THIS IS THE FIRST ENTRY WHOSE COMMITTED
RECORD IS THE PIPELINE'S.** The `witness` job of run 35514833652 ran 14:44:08–14:44:27Z. Its build step
read the run's one approval under `actions: read` — approver `decentcybersecurity`, 14:43:10Z, comment
*Approved on the readiness report at the release commit:* followed by the report's URL at `89d2c707`,
so the builder lifted that URL into `review_file` from the approval's own words and nobody designated
anything — read deployment `6553754047`'s four statuses under `deployments: read`, fetched the
attestation store by the wheel's digest, was handed `--attestation-bundles`, and at 14:44:17Z wrote
`witness-3.0.1.json for v3.0.1 (2 files, 1 approval(s))`. Its verify step, `gates/witness_verify.py
witness-3.0.1.json --download --assets assets` with a token, read `VERIFIED witness-3.0.1.json (3.0.1,
against the index and Release and the assets under assets)` — `1 witness record(s), 0 disagreeing` —
where run 35200069387's read `DISAGREES` on an empty comment and `v2.1.2`'s failed on a missing instant.
Its attach step uploaded the record to the Release at 14:44:23Z as the ninth asset, 2 188 bytes, sha256
`32cd277e2f68157de29188a0d59ef136ebe19dc63b3b37788934ea5049292acd`. The witness round downloaded the
nine assets, verified that record offline against them and online with `--download` — both read
`VERIFIED`, `0 disagreeing` — and committed it as `releases/witness/3.0.1.json`, the same bytes;
`tests/test_cdm_witness.py` holds the file to that digest and the three dated paragraphs of 2026-09-17
to what this run did. The rule of 2026-09-17 — a non-empty approval comment naming what the approval
was taken on — was followed and is what made the pipeline's own record acceptable to its own verifier;
the record's `comment` and `review_file` name the same document.

**THE CONDITION 4 RUN, IN THE DOCUMENTED ENVIRONMENT, AND WHAT THE NUMBERS DO AND DO NOT SHOW.** The
gate's condition 1 ran 13:53:59–14:00:45Z, six minutes and forty-six seconds, and the build job's
condition 4 ran 14:05:07–14:18:37Z, thirteen minutes and thirty seconds — both in the documented
environment now, on two runners. The `v3.0.0` run's pair read 811 s and 1262 s. So the spread between
the two jobs' suite runs did not close with the corrective, and the honest reading is that a shared
runner accounts for a factor of two on its own between two executions of one suite an hour apart; the
451 s the 3.0.1 section attributes to the import closure is the part the measurement isolates — a
0.37 s cost paid by every worker start, reproduced on one machine — and not the whole of the difference.
A wall-clock budget of 10 s across three worker start-ups therefore has a thinner margin on a runner than
its module assumed, and that is a fact about the budget rather than about the environment, recorded
here for the round that next touches `tests/test_cdm_parser_isolation.py`; no budget moved in this arc.

**This round writes nine files**, and the tenth is the test module that states the ledger count.
`releases/witness/3.0.1.json` is new; `PUBLICATION.md` gains this entry; `docs/docs/changelog.mdx` gains a
dated measurement beside the 3.0.1 paragraph whose present-tense sentence about what the index serves
stopped being true at 14:43:35Z; `packages/cdm/synapse_cdm/MIGRATIONS.md` records the act, opens the
pending section this one-file change makes necessary, and rewrites its dated witness paragraphs to say
what run 35514833652 did; `releases/witness/README.md` and `docs/docs/security/release-pipeline.mdx` are
rewritten the same way, and the README says what `review_file` means for 3.0.1; `SECURITY.md`'s SBOM row
gains the v3.0.1 asset sizes; `docs/audit-remediation-report.md`'s release note records the upload;
`tests/test_cdm_witness.py` moves the assertion that held the directory to two hand-built records to two
and the pipeline's first, by digest. `tests/test_cdm_publication.py` moves its docstring and its number
vocabulary, which ended at twenty. **Nothing under `packages/cdm/synapse_cdm/` moved except
`MIGRATIONS.md`**, and `PACKAGE_VERSION` still reads `3.0.1`.

**What this entry does not claim.** It does not claim to know which test failed at `v3.0.0`'s condition
4: the step discarded the name, the identity is a reconstruction from timings and an import measurement,
the 3.0.1 section says so, and the repaired step is what stops the next one from being a reconstruction.
It does not claim the corrective made the build job's suite run as fast as the gate's — the paragraph
above gives both numbers. It does not claim the documentation site is deployed or gated: the deployment
record's next row is the next round's, and `docs.synapsecommand.com` at the time of writing serves what
entry 20's second deploy put there. It does not claim the simple index's lag has a length. It does not
...
| ~~`docs.synapsecommand.com` is served by `5a908754`~~ | entry 8, entry 15 | `gates/deploy_record.py`, by bytes | **superseded three times over while this row went on reading `holds`** — by `6ec27291` at 2026-09-05 22:26:51Z, by `2fc4ab7b` at 2026-09-06 09:53:41Z and by `7031ab1f` at 2026-09-07 12:05:26Z | **PROTOCOL-GATED** — struck 2026-09-07 by the 2.0.0 witness round, three supersessions late. The three dates are entry 8's own table and the entries that made them, not readings taken here; **the habit is the one the 1.4.0 version row below names, and this is its second instance** — entry 18's first finding |
| `docs.synapsecommand.com` is served by `7031ab1f` | entry 8, entry 18 | `gates/deploy_record.py`, by bytes | holds — 5/5 pages from the domain byte-identical to `7031ab1f`'s own `pages.dev` URL and 5/5 differing from `2fc4ab7b`, read **2026-09-07T12:11:09–12:11:15Z** | **PROTOCOL-GATED** |
...
| ~~the distribution on the index is 1.6.0~~ | `MIGRATIONS.md`, `### 1.6.0`; entry 15 | `GET /pypi/synapse-cdm/json` | **superseded 2026-09-05 by 1.7.0, then 2026-09-06 by 1.8.0, then 2026-09-07 by 2.0.0, and this row went on reading `holds` through all three** — struck 2026-09-07 by the 2.0.0 witness round; the section cited is a released heading and stays a durable address | dated **2026-09-05T11:12:43Z**. **SUITE-GATED at one remove**, on the same reading as every version row before it. Entries 16 and 17 recorded their own index readings and neither reached this table, which is the 1.4.0 row's failure repeated — entry 18's first finding |
| the distribution on the index is 2.0.0 | `MIGRATIONS.md`, `### 2.0.0`; entry 18 | `GET /pypi/synapse-cdm/2.0.0/json` under entry 12's probe protocol, and the simple index in both its HTML and JSON forms | holds — `200`, `info.version` `2.0.0`, two files, read **2026-09-07T11:59:25.117Z**; the simple index at 11:59:15.289Z carries **twenty-four** anchors whose last two are 2.0.0's and lists **twelve** versions | dated **2026-09-07T11:59:15–11:59:25Z**. **SUITE-GATED at one remove**, on the same reading as every version row before it: the suite ties the tag to the tree's `PACKAGE_VERSION` and nothing in it reads the index. The legacy API's *latest-version* form (`GET /pypi/synapse-cdm/json`) answered `1.8.0` in a read bounded to 11:56:20–11:57:19Z and `2.0.0` at 11:59:25.196Z — a bounded observation outside the probe protocol, not a measurement; entry 18 |
| the version the docs site's own changelog page states | deployment section, and `docs/docs/changelog.mdx` | `gates/deploy_record.py`'s served-version witness — `GET https://docs.synapsecommand.com/changelog/` under entry 12's probe protocol, the sentence parsed, compared with `version.py`'s `PACKAGE_VERSION` | **agree** — the page states `1.5.0` and the tree declares `1.5.0`, read **2026-09-04T21:48:54Z**. **DISAGREE** — page `1.5.0`, tree `1.6.0`, read **2026-09-05T11:02:50Z**: the witness's first disagreement, on the first release after it was written, and the reading it exists for. **agree** — page `1.6.0`, tree `1.6.0`, read **2026-09-05T11:18:08Z**, after this round deployed `5a908754` from the tagged tree — entry 15 . **Entry 17 recorded a DISAGREE and an AGREE for 1.8.0 and neither reached this cell.** **DISAGREE** — page `1.8.0`, tree `2.0.0`, read **2026-09-07T11:57:25Z**, the witness's fourth disagreement and its fourth release. **agree** — page `2.0.0`, tree `2.0.0`, read **2026-09-07T12:11:15Z**, after this round deployed `7031ab1f` from the tagged tree — entry 18 | **PROTOCOL-GATED, and a WITNESS rather than a check: it cannot fail the gate.** What the site serves is a fact about the last deploy and never about this tree, so a disagreement is the ordinary condition of every hour between a release and its deploy and refusing it would be red for a reason the tree cannot fix. The parser is tested offline over a saved page in `tests/test_cdm_deploy_record.py`; nothing in the suite reaches the network. **The round that added it expected this reading to DISAGREE and it agrees** — see `MIGRATIONS.md` at the maintenance round of 2026-09-05 |

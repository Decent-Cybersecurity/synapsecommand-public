...
above: `version.compatible()` decides by direction and by frozen evidence — a current reader
accepts an older object of the same major that the frozen schemas show validating — and
...
- **A finding that requires the operator to load a hostile adapter.** `load_adapter` accepts
  `module:ClassName` and imports it (`packages/cdm/synapse_cdm/adapter.py:654`). Choosing which
...
| Resource isolation of the conformance worker | **active** | **With a stated limit (audit remediation F06, 2026-09-20).** Checks H and N run in a spawned subprocess with an enforced per-case deadline, a kill escalation, an output cap and a restart cap on every platform (`tests/test_cdm_parser_isolation.py`). The memory/CPU envelope of that worker (`suite.ResourceLimits`, `--memory-limit-bytes`, `--cpu-limit-seconds`, applied as `RLIMIT_AS`/`RLIMIT_CPU`) is **Linux-only**: macOS refuses a finite `RLIMIT_AS` with `EINVAL` and accepts but does not enforce `RLIMIT_CPU` (measured 2026-09-20 on 26.5.2 / CPython 3.14.7), and Windows has no `resource` module. An explicit request the platform cannot enforce is refused up front — `UnsupportedResourceLimit`, exit 2 — never applied as a best effort; `tests/test_cdm_resource_envelope.py` asserts the enforcement on Linux and the refusal elsewhere. The worker is not a sandbox: no seccomp, namespace, filesystem or network restriction. `docs/docs/security/deployment-envelope.mdx` is the layer-by-layer statement. |
| Streaming input | **not provided** | none exists, and none is advertised. `to_cdm` takes one complete payload inside `max_input_bytes`; chunk framing, partial messages and reassembly are not implemented and backpressure is not applicable, in those words in conformance check M (`suite.STREAMING_STATUS`), which is `SKIP` with `declared_inapplicable: true` for all fourteen and never `PASS`. `docs/docs/security/deployment-envelope.mdx` §4. |
| Verification of the `integrity` block | **not provided** | the `integrity` field on every CDM object is a data container for a signature this package does not make and does not verify; nothing outside `models.py` reads or sets it (`tests/test_cdm_resource_envelope.py` walks the syntax trees), and a record carrying a block is exactly as verified as one carrying none. The signing row below records why none exists. |
...
| Dependency review on pull requests | **active** | **With a stated limit.** `.github/workflows/dependency-review.yml`, `fail-on-severity: high`, no `allow-ghsas`. **It exercises only on pull requests**, and the SOIF campaign pushes directly — so the layer that covers the dependency set on every push is the `pip-audit` row below. **Corrected 2026-09-16: that sentence named one row where two exist.** `pip-audit` is Python-only; the layer that covers `docs/package-lock.json` on every push is the npm audit row after it, which this table lacked until this date. `docs/docs/security/supply-chain.mdx` §2 says which green means what. |
...
| npm dependency audit over `docs/` | **active** | `.github/workflows/ci.yml`, job `docs-audit`, on every push since round PB (2026-09-08): `npm ci --prefix docs` from the committed lock, `npm audit --prefix docs --audit-level=high` verbatim for the log, then a step that reads `npm audit --json` and fails on any high or critical ADVISORY that is not excepted — the allowlist derived from `security/exceptions/` by the same `gates/codeql_gate.py --emit-pip-audit-ignores` the `supply-chain` job uses, never typed into the workflow — and last `npm --prefix docs run ci`, so a lock that clears an advisory and breaks the site is not green. **Absent from this table until 2026-09-16**, although both `image-size` exception files named the job as their enforcing mitigation. Reading, 2026-09-16, taken with the job's own steps over the committed lock: `advisories: 1; excepted and present: []; excepted and absent: []`, `OK` — the one is `GHSA-w5hq-g745-h8pq` (`uuid`, moderate, reachable only through `webpack-dev-server`, which no build uses), below the floor; `npm audit --json` → high 0, critical 0, moderate 17, sixteen of which are dependents inheriting that one advisory. **Reading, 2026-09-17, after `uuid` took the seventh floor in the row below:** `advisories: 0; excepted and present: []; excepted and absent: []`, `OK`; `npm audit --json` → high 0, critical 0, moderate 0, total 0; `npm audit --audit-level=high` exits 0 on `found 0 vulnerabilities` — the first reading of this job over a lock with nothing to report at any severity. |
| Pinned floors on transitive npm dependencies of `docs/` | **active** | `docs/package.json`, `overrides`: seven entries — `fast-uri ^3.1.7`, `image-size ^2.0.4`, `js-yaml ^4.3.2`, `qs ^6.16.0`, `serialize-javascript ^7.1.1`, `svgo ^3.3.5`, `uuid ^11.1.1` — the first six each the repair for a high npm advisory in a package reachable only under `@docusaurus/*@3.10.2`, the newest Docusaurus there is, so no bump of a direct dependency reaches it, and the seventh (2026-09-17; this row said six entries until then) the same repair for the one moderate advisory the reading above left standing, `GHSA-w5hq-g745-h8pq`, whose first patched version, 11.1.1, `sockjs` 0.3.24's `^8.3.2` could never resolve — the "no fix" every earlier reading reported was npm's `fixAvailable: false` over the tree, not the registry, where 11.1.1 has existed since 2026-04-29. Overrides and never `npm audit fix`, which downgraded `qs` into its vulnerable range when it was tried; the rationale for each pin is dated in `packages/cdm/synapse_cdm/MIGRATIONS.md` (rounds PB and PD for the first five, the audit's npm records of 2026-09-16 for `image-size` and of 2026-09-17 for `uuid`). The row above is what proves each pin holds: a floor that regresses is an advisory that job fails on — or, for `uuid` alone, one it reports and does not fail on, since the job's floor is npm's severity label and `uuid`'s is moderate; `tests/test_cdm_security_policy.py` holds every entry of the block to this row, so a pin this row does not name is red. |
| CodeQL, with a gate that fails the build | **active** | `.github/workflows/codeql.yml`, advanced setup, `security-extended`, matrix `python` and `javascript-typescript`, weekly schedule; `gh api .../code-scanning/default-setup` → `not-configured`, which is what advanced setup requires. `gates/codeql_gate.py` reads the SARIF the run produced and exits non-zero on any finding at `security-severity >= 7.0` — M's ruling of 2026-09-07: HIGH and CRITICAL block, and a 9.0 threshold must not mean 7.0–8.9 is ignored. **No real analysis has been read yet**: the first run is round P7's to record. **Read 2026-09-08 (round P7's Act 0), and this row is the record it promised.** The first run, 34212170555 on `c52e496`, was RED: four findings at or above the 7.0 threshold — one `py/incomplete-url-substring-sanitization` (7.8) and three in `docs/scripts/` (`js/incomplete-sanitization` x2 at 7.8, `js/file-system-race` at 7.7). None was in the shipped package. Round PC fixed all four at their sites and wrote no exception; the run on PC's commit `e540de8`, 34220573582, is GREEN. `security/exceptions/` still holds zero exception files. **Corrected 2026-09-16: it did not stay that way, and this sentence stood while it was false.** Round PB wrote two files into it later on 2026-09-08 — `image-size`'s two npm advisories, neither of them a CodeQL finding — and the audit of 2026-09-16 deleted them on their own removal trigger; the exceptions row below carries the dated history. No CodeQL finding has ever been excepted. The gate is now also a stage of the release pipeline itself (`.github/workflows/publish.yml`, job `gate`), where a commit with NO analysis is a failure and not a pass. |
| SBOM, two formats | **active** | **Corrected 2026-09-16.** The reading below this sentence was taken in round P6 and named `rc-build.yml`, whose SBOMs were then syft over the wheel file — the shape round PS (2026-09-08) read as one SPDX package and zero CycloneDX components. The ACTIVE control is `.github/workflows/publish.yml`, job `build`: syft (SPDX-JSON and CycloneDX-JSON) over the clean-install venv created from the gated wheel, `cyclonedx-py environment` over the same venv as a cross-check, and four refusals — an empty SPDX document, an empty CycloneDX document, no `synapse-cdm` entry, a version that is not `PACKAGE_VERSION`. Both SBOMs are attached to the Release and attested: the v2.1.2 assets are `synapse_cdm.spdx.json` (110 994 bytes) and `synapse_cdm.cdx.json` (69 254 bytes); re-read 2026-09-17 off the v2.2.0 Release, the two assets carry the same names at the same sizes, 110 994 and 69 254 bytes, with digests `759d2272…` and `af4de5f3…` that `releases/witness/2.2.0.json` records; re-read 2026-09-20 off the v3.0.1 Release, again the same names at the same sizes, 110 994 and 69 254 bytes, with digests `6cee32b1…` and `2cfb3088…` that `releases/witness/3.0.1.json` — the pipeline's own record — records. `rc-build.yml`'s `build` job takes the same subject since 2026-09-16. The P6 reading, kept: `.github/workflows/rc-build.yml`, job `build`: syft over the gated wheel producing SPDX-JSON and CycloneDX-JSON, plus `cyclonedx-py environment` as a cross-check. Both are workflow artefacts and both are attested. |
| Build attestation, no long-lived key | **active** | `.github/workflows/rc-build.yml`, job `attest`: `actions/attest-build-provenance` (Sigstore, OIDC) over the wheel, the sdist and both SBOMs, then `gh attestation verify` on each as the proof. `id-token: write` and `attestations: write` are on that job and nowhere else in the repository; there is no signing key anywhere. **Not yet exercised on GitHub** — `rc-build.yml` is `workflow_dispatch` and its first run is round P7's Act 0. **CORRECTED 2026-09-08 (round P7): that run cannot happen yet, and the reason is structural rather than an omission.** GitHub refuses to dispatch a workflow whose file is not on the DEFAULT branch, and `rc-build.yml` exists only on `soif/1.0` — `gh workflow run rc-build.yml` answers `HTTP 404: workflow rc-build.yml not found on the default branch`. A workflow registers by RUNNING, and a `workflow_dispatch`-only workflow off the default branch can never get a first run. So its first dispatch is a POST-RELEASE reading, after the release round fast-forwards `main`. What IS exercised, on M's ruling of 2026-09-08: the attestation stage moved into `publish.yml` as the `attest` job, which a branch dispatch DOES reach because `publish.yml` is already on `main` — baseline dispatch 34220688516, green. `id-token: write` and `attestations: write` are on that job alone. **Corrected 2026-09-16: two sentences in this row overstate the grant by one job.** "On that job and nowhere else in the repository" (of `rc-build.yml`'s `attest`) and "on that job alone" (of `publish.yml`'s `attest`) are both false for `id-token: write`. `attestations: write` is on the two `attest` jobs (`rc-build.yml`, `publish.yml`) and nowhere else; `id-token: write` is on those two and on `publish.yml`'s `publish` job, which mints the PyPI Trusted Publishing token with it. `tests/test_cdm_security_policy.py` derives the holders from the workflows and requires this row to name each one. |
| Documented, time-bounded security exceptions | **active** | `security/exceptions/` with `schema.json`, a README and **zero exception files today**. **Corrected 2026-09-16: that "today" was the morning of 2026-09-08, and the sentence was false by the evening.** Round PB wrote two files the same day — `GHSA-w3rx-r6r6-pgpr` and `GHSA-5p2g-fcmc-qvqq`, `image-size`'s ICNS and JXL/HEIF denial-of-service advisories, high, reachable only at documentation build time, `expiry` 2026-11-07 — and they stood, uncorrected here, until 2026-09-16, when `image-size` 2.0.3 (published 2026-09-14 from the project's new home on Codeberg) turned out to carry the fix for exactly those three parsers and both files were DELETED as their `upstream_status` required, with `docs/package-lock.json` moved to 2.0.4 by an `overrides` entry. The directory holds zero exception files again, and this time the sentence is a second reading rather than the first one surviving. `gates/codeql_gate.py`, the `pip-audit` step and the `docs-audit` job (its own row above; this row said two consumers until 2026-09-16) all derive their allowlist from that directory at run time — no list is typed into any workflow, and `tests/test_cdm_security_exceptions.py` asserts that none ever is. An `expiry` in the past fails the whole suite, unconditionally, on the day it passes. |

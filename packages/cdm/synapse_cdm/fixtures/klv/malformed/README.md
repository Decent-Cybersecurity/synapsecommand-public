# Malformed klv payloads — every one synthetic, every one refused
...
```bash
synapse conformance run --adapter stanag4609 --require H
```
...
|---|---|
| `truncated_payload.klv` | a packet cut off inside its local set |
| `declared_length_longer_than_the_packet.klv` | §21's `oversized declared field`: the BER length declares 255 octets and the packet carries fewer |

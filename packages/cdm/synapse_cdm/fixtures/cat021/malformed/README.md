# Malformed cat021 payloads — every one synthetic, every one refused
...
```bash
synapse conformance run --adapter cat021 --require H
```
...
|---|---|
| `truncated_payload.cat021` | a data block cut off inside its records |
| `declared_length_longer_than_the_block.cat021` | §21's `invalid length`: LEN declares 65 535 octets and the block is shorter |

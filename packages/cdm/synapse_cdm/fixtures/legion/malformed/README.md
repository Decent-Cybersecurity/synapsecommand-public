# Malformed legion payloads — every one synthetic, every one refused
...
```bash
synapse conformance run --adapter legion --require H
```
...
|---|---|
| `a_truncated_record.json` | a record carrying only its first few fields — well-formed JSON that is not a well-formed Legion object, which is the truncation case for a document-shaped source |

...
class UnknownFields(str, enum.Enum):
    ...
    carrier for an unknown key in the decoded twin, and those are two true facts that one enum
    value cannot hold. The accompanying `unknown_fields_basis` is where the distinction is stated,
    in the adapter's own words, which is the same arrangement `Limits.absent_because` uses for a

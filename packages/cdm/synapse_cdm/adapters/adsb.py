    ...
    metadata = AdapterMetadata(
        ...
        maturity=Maturity(
            ...
            basis="L4 ROUNDTRIP VERIFIED, from evidence that runs today. The harness's "
                  ...
                  "(`ROUNDTRIP_TOLERANCE`), so `synapse conformance run --adapter adsb` computes "
                  "E = PASS and L4 is the suite's own reading rather than this declaration's. The "
                  "adapter's own statement of the same claim is "
                  "tests/test_cdm_adsb_adapter.py::test_the_ingest_round_trip_is_byte_exact. L5 "
                  "is eligible and not declared: the rung above this one rests on `M` (streaming) "

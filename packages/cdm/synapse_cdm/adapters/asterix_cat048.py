...
class AsterixCat048Adapter(Adapter):
    ...
    metadata = AdapterMetadata(
        ...
        format=FormatRef(name="EUROCONTROL ASTERIX Category 048 — Monoradar Target Reports",
                         version="Edition 1.32"),
        ...
        maturity=Maturity(
            ...
            basis="L3 PROVENANCE VERIFIED, from evidence that runs today. The harness's "
                  ...
                  "conformance run --adapter cat048` computes E = PASS and `maturity_eligible` "
                  "reads above this declaration. L4 is NOT declared (ruled 2026-09-20, audit "
                  "remediation F02, register §4): 'applicable information survives source -> CDM "
                  "-> source' is the claim the heuristic cannot prove, so the declaration stops at "
                  "the last rung whose evidence is path-bound and regains L4 only when this "
                  "adapter's field mappings are declared and the ledger reports no loss. The "
                  "adapter's own statement of the round-trip claim is "
                  "tests/test_cdm_asterix_cat048_adapter.py::test_every_fixture_round_trips_byte_exactly. "
                ...
                declared_because={
                    "max_input_bytes": LimitBasis(
                        ...
                        source=(
                            ...
                            "a TWO-OCTET field and §4.6.2 makes it the total length of the "
                            "whole block including CAT and LEN — quoted at "
                            "`adapters/asterix_cat048.py`'s `parse_block`, where it is read as "
                            "`codec.read_unsigned(data, 1, 2)`. Two octets cannot express more than "
        ...
        limitations=[
            ...
            "edition of this category defines its contents",

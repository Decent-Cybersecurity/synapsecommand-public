...
class TakAdapter(Adapter):
    ...
    #: `fixtures/tak/spec/tak_terms.json` — `license_class_supported_by_these_readings`
    metadata = AdapterMetadata(
        id="tak",
        name="Cursor-on-Target",
        ...
        maturity=Maturity(
            ...
            basis="L3 PROVENANCE VERIFIED, from evidence that runs today. The harness's "
                  ...
                  "finds no source value missing from any parsed twin, so `synapse conformance run "
                  "--adapter tak` computes E = PASS and `maturity_eligible` reads above this "
                  ...
                  "tests/test_cdm_tak_adapter.py::test_ingest_round_trip_loses_no_source_value. L5 "
                  "is likewise not declared: the rung above L4 rests on `M` (streaming) being "
                ...
                declared_because={
                    ...
                    "max_input_bytes": LimitBasis(
                        ...
                        source=(
                            ...
                            "bounds what internal entities may expand it to. This is an "
        ...
        limitations=[
            ...
            "document in this repository — `fixtures/tak/spec/tak_terms.json` records the "
            "Base-Event Schema's own version as its publisher states it, which is a reading "
            "of that page and not a statement that this implementation targets it, so "

...
# `fixtures/ais/`, goldens included (it nests seven), and every walker stays under two
...
class AisAdapter(Adapter):
    ...
    #: `fixtures/ais/spec/ais_terms.json` — `license_class_supported_by_these_readings`
    metadata = AdapterMetadata(
        ...
        maturity=Maturity(
            ...
            basis="L3 PROVENANCE VERIFIED, from evidence that runs today. The harness's "
                  ...
                  "octet under the declared `bytes` tolerance (`ROUNDTRIP_TOLERANCE`), the two "
                  "fixtures that carry an NMEA TAG block being compared against "
                  "`roundtrip_reference(raw)` — the sentences without the receiver's own "
                  "annotation, which the harness names in every report, so `synapse conformance "
                  "run --adapter ais` computes E = PASS and `maturity_eligible` reads above this "
                  "declaration. L4 is NOT declared (ruled 2026-09-20, audit remediation F02, "
                  "register §4): 'applicable information survives source -> CDM -> source' is the "
                  "claim the heuristic cannot prove, so the declaration stops at the last rung "
                  ...
                  "likewise not declared: the rung above L4 rests on `M` (streaming) being "
        ...
        capabilities=Capabilities(
            ...
            limits=Limits(
                ...
                declared_because={
                    "max_depth": LimitBasis(
                        ...
                        source=(
                            "An AIS message does not nest, but `to_cdm` also takes the parsed twin "
                            "— a dict, or its JSON text — and the walk after that parse recurses "
                            "once per level; no JSON document shipped under `fixtures/ais/`, "
                            "goldens included, nests more than seven containers, and the deepest "
                            "twin `to_cdm` is handed nests four. 64 is chosen on 2026-09-17 for the "
                            "reason `tak` chose it on 2026-09-16: every walk after the parse "
                            "recurses once per level, and on CPython 3.11 `json.loads` itself "
                            "recurses once per container and raises `RecursionError` a little under "
                            "a thousand containers deep (the reading is in `adapter.InputTooDeep`), "
                            "so the bound is read off the characters before the decoder runs. Nine "
                            "times the deepest JSON document shipped beside the fixtures, and every "
                            "walker stays under two hundred frames. This is an IMPLEMENTATION CAP "
                            "(`AIS_MAX_DEPTH`, `adapters/ais.py`) and is NOT the format's normative "
                            "maximum."),
                        enforced_at=(
            ...
            unknown_fields_basis=(
                ...
                "consume is parked verbatim under `source_extras`. The six-bit armoured payload has "
        ...
        limitations=[
            ...
            "this repository — the format has two publishers, ITU-R (M.1371) and NMEA (0183), "
            "and `fixtures/ais/spec/ais_terms.json` records both without naming the edition "
            "this implementation targets, so `format.version` is null rather than guessed",
            "message types 1, 2, 3, 4, 5, 18, 19 and 21 are in scope; every other type is "
            "named in FORMAT_COVERAGE.md with the reason it is out",
            "type 24 static data is out of scope, so a vessel's name and call sign arrive in "
            "no message this adapter reads",

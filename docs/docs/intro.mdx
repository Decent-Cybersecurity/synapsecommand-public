...
Nine integration adapters are shipped and harness-verified — PNTMAP
...
detections (bidirectional, byte-exact), and ASTERIX category 048
monoradar target reports (bidirectional, byte-exact) — with more
landing next: the other ASTERIX categories (062 system tracks, 023
service status) and simulation feeds.

Without a canonical model in the middle, N adapters means N(N−1)/2
translations and N private notions of what "a contact" is — thirty-six
and nine as of today — and the integration layer becomes the place

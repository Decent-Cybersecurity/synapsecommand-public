...
Eight integration adapters are shipped and harness-verified — PNTMAP
...
(bidirectional), and STANAG 4607 / AEDP-4607 Edition A GMTI radar
detections (bidirectional, byte-exact) — with more landing next: the
other ASTERIX categories (062 system tracks, 023 service status) and
simulation feeds.

Without a canonical model in the middle, N adapters means N×(N−1)
translations and N private notions of what "a contact" is — and the
integration layer becomes the place where meaning is quietly lost. With

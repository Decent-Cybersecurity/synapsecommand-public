...
Twelve integration adapters are shipped and harness-verified — PNTMAP
...
byte-exact), and ASTERIX category 023 CNS/ATM ground station and
service status reports (bidirectional, byte-exact).

Without a canonical model in the middle, N adapters means N(N−1)/2
translations and N private notions of what "a contact" is — sixty-six
and twelve as of today — and the integration layer becomes the place

...
| `$id` | `urn:synapsecommand:cdm:2.1.0:cdm_object` |
| CDM schema version | `2.1.0` |
| SHA-256 of source | `28b5083ef581e6ae6e21d402b4ce7c258e7efcbdba57dcf558ab9cf90b2b3e5c` |
...
| `schema_version` | string | no | Semver of the CDM this object was written against. Default `"2.1.0"`. (pattern `^(0\|[1-9][0-9]*)\.(0\|[1-9][0-9]*)\.(0\|[1-9][0-9]*)$`) |
...
| `symbol` | string \| null | no | MIL-STD-2525D SIDC, 20 digits. None when the source states no symbol — see symbology.sidc_from_affiliation() for deriving one. Default `null`. (pattern `^[0-9]{20}$`) |
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `entity_id` | string (uuid) | **yes** | Must also appear in the event's related_entities — see Event._oes_relations. |
